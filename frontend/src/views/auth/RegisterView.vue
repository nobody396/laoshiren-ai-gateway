  ...
  <AuthLayout>
    <div class="space-y-6">
      ...
      <OAuthProviderSection
        ...
        :disabled="isLoading"
        :invitation-code="formData.invitation_code"
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div>
          ...
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="mail" size="md" class="text-gray-400 dark:text-dark-400" />
            </div>
            <input
              id="email"
        ...
        <div>
          ...
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="lock" size="md" class="text-gray-400 dark:text-dark-400" />
            </div>
            <input
              id="password"
        ...
        <div v-if="invitationCodeEnabled">
          ...
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="key" size="md" :class="invitationValidation.valid ? 'text-green-500' : 'text-gray-400 dark:text-dark-400'" />
            </div>
            <input
              id="invitation_code"
        ...
        <!-- Referral Code Input (Optional, always shown)（暂时隐藏） -->
        <div>
          <label for="referral_code" class="input-label">
            {{ t('auth.referralCodeLabel') }}
            <span class="ml-1 text-xs font-normal text-gray-400 dark:text-dark-400">({{ t('common.optional') }})</span>
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="userPlus" size="md" class="text-gray-400 dark:text-dark-400" />
            </div>
            <input
              id="referral_code"
        ...
        <div v-if="promoCodeEnabled">
          ...
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="gift" size="md" :class="promoValidation.valid ? 'text-green-500' : 'text-gray-400 dark:text-dark-400'" />
            </div>
            <input
              id="promo_code"
...
import OAuthProviderSection from '@/components/auth/OAuthProviderSection.vue'
...
const siteName = ref<string>('老实人AI')
const linuxdoOAuthEnabled = ref<boolean>(false)
const googleOAuthEnabled = ref<boolean>(false)
const googleOAuthProviderName = ref<string>('Google')
...
interface RegisterOAuthProvider {
  provider: 'linuxdo' | 'google' | 'github'
  label: string
}

const oauthProviders = computed<RegisterOAuthProvider[]>(() => {
  if (backendModeEnabled.value) {
    return []
  }
  const providers: RegisterOAuthProvider[] = []
  if (linuxdoOAuthEnabled.value) {
    providers.push({ provider: 'linuxdo', label: t('auth.linuxdo.signIn') })
  }
  if (googleOAuthEnabled.value) {
    providers.push({
      provider: 'google',
      label: t('auth.oauth.signInWith', { provider: googleOAuthProviderName.value || 'Google' })
    })
  }
  if (githubOAuthEnabled.value) {
    providers.push({ provider: 'github', label: t('auth.oauth.signInWith', { provider: 'GitHub' }) })
  }
  return providers
})
...
onMounted(async () => {
  try {
    ...
    googleOAuthEnabled.value = settings.oidc_oauth_enabled
    googleOAuthProviderName.value = settings.oidc_oauth_provider_name || 'Google'
    ...
    // Read referral/affiliate code from URL parameters (?aff=XXXX takes priority over ?ref=XXXX)
    const affParam = route.query.aff as string
    const refParam = route.query.ref as string
    if (affParam || refParam) {
      formData.referral_code = affParam || refParam
    }
  } catch (error) {
