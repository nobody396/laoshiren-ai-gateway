...
```bash
curl -fsSL 'https://laoshirenai.com/auto-config/install.sh?v=0.7.32' | bash
```
...
- 仅在所选客户端缺失时安装对应 CLI
- 写入对应配置文件
- 使用当前 Codex API Key 请求 `/v1/models`，只写入该分组实际开放的模型目录
...
```powershell
irm https://laoshirenai.com/auto-config/install.ps1?v=0.7.32 | iex
```
...
```bash
curl -fsSL 'https://laoshirenai.com/auto-config/install.sh?v=0.7.32' | bash -s -- --api-key YOUR_CLAUDE_KEY --tools claude
```

Windows PowerShell（管道模式通过环境变量传参）：

```powershell
$env:LAOSHIRENAI_CLAUDE_API_KEY='YOUR_CLAUDE_KEY'; $env:LAOSHIRENAI_TOOLS='claude'; irm https://laoshirenai.com/auto-config/install.ps1?v=0.7.32 | iex
```
...
```bash
curl -fsSL 'https://laoshirenai.com/auto-config/install.sh?v=0.7.32' | bash -s -- --codex-api-key YOUR_CODEX_KEY --tools codex --base-url https://api.laoshirenai.com
```

Windows PowerShell（管道模式）：

```powershell
$env:LAOSHIRENAI_CODEX_API_KEY='YOUR_CODEX_KEY'; $env:LAOSHIRENAI_TOOLS='codex'; $env:LAOSHIRENAI_BASE_URL='https://api.laoshirenai.com'; irm https://laoshirenai.com/auto-config/install.ps1?v=0.7.32 | iex
```
...
```bash
curl -fsSL 'https://laoshirenai.com/auto-config/install.sh?v=0.7.32' | bash -s -- --api-key YOUR_CLAUDE_KEY --codex-api-key YOUR_CODEX_KEY
```

Windows PowerShell（管道模式）：

```powershell
$env:LAOSHIRENAI_CLAUDE_API_KEY='YOUR_CLAUDE_KEY'; $env:LAOSHIRENAI_CODEX_API_KEY='YOUR_CODEX_KEY'; irm https://laoshirenai.com/auto-config/install.ps1?v=0.7.32 | iex
```
...
```bash
curl -fsSL 'https://laoshirenai.com/auto-config/install.sh?v=0.7.32' | bash -s -- --api-key YOUR_CLAUDE_KEY --base-url https://api.laoshirenai.com
```
...
```powershell
$env:LAOSHIRENAI_TOOLS='claude'; $env:LAOSHIRENAI_CLAUDE_API_KEY='YOUR_KEY'; irm https://laoshirenai.com/auto-config/install.ps1?v=0.7.32 | iex
```
