...
// StartBackup starts a background backup job and returns immediately with the running record.
...
func (s *BackupService) executeBackup(record *BackupRecord, objectStore BackupObjectStore) {
	...
	if err != nil {
		...
		record.Progress = ""
		if gzipErr != nil {
			record.ErrorMsg = fmt.Sprintf("gzip/dump failed: %v", gzipErr)
		} else {
...
// StartRestore starts a background restore job and returns immediately with the updated record.
