...
func (r *groupRepository) GetByIDLite(ctx context.Context, id int64) (*service.Group, error) {
	...
	if flags, err := r.loadChatbotEnabled(ctx, []int64{out.ID}); err == nil {
		out.ChatbotEnabled = flags[out.ID]
	}
	return out, nil
}
...
func (r *groupRepository) applyChatbotEnabled(ctx context.Context, groups []service.Group) {
	if len(groups) == 0 {
		return
	}
	...
	flags, err := r.loadChatbotEnabled(ctx, ids)
	if err != nil {
		return
	}
	for i := range groups {
		groups[i].ChatbotEnabled = flags[groups[i].ID]
	}
}

func (r *groupRepository) loadChatbotEnabled(ctx context.Context, groupIDs []int64) (map[int64]bool, error) {
...
func (r *groupRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool) ([]service.Group, *pagination.PaginationResult, error) {
	...
	if strings.ToLower(strings.TrimSpace(params.SortBy)) == "account_count" {
	...
	r.applyChatbotEnabled(ctx, outGroups)
...
func (r *groupRepository) listWithAccountCountSort(ctx context.Context, q *dbent.GroupQuery, params pagination.PaginationParams, total int) ([]service.Group, *pagination.PaginationResult, error) {
	...
	for i, row := range rows {
		groupIDs[i] = row.ID
		entries = append(entries, sortEntry{id: row.ID, sortOrder: row.SortOrder})
	}
	...
	for i := range entries {
		entries[i].accountCount = counts[entries[i].id].Total
	}

	sortOrder := params.NormalizedSortOrder(pagination.SortOrderDesc)
	tieLess := func(a, b sortEntry) bool {
	...
	sort.SliceStable(entries, func(i, j int) bool {
		if entries[i].accountCount == entries[j].accountCount {
			return tieLess(entries[i], entries[j])
		}
	...
	if len(page) == 0 {
		return []service.Group{}, paginationResultFromTotal(int64(total), params), nil
	}
...
func (r *groupRepository) GetGroupCacheStats(ctx context.Context, groupIDs []int64, startTime time.Time) (map[int64]service.GroupCacheStats, error) {
	stats := make(map[int64]service.GroupCacheStats, len(groupIDs))
	if len(groupIDs) == 0 {
		return stats, nil
	}
	...
	for rows.Next() {
		var stat service.GroupCacheStats
		if err := rows.Scan(
			&stat.GroupID,
			&stat.Requests,
			&stat.InputTokens,
		...
		cacheableInputTokens := stat.InputTokens + stat.CacheCreationTokens + stat.CacheReadTokens
		if cacheableInputTokens > 0 {
			stat.HasData = true
			stat.HitRate = float64(stat.CacheReadTokens) / float64(cacheableInputTokens)
			stat.HitRatePct = stat.HitRate * 100
		}
