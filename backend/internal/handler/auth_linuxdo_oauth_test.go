...
func TestOAuthRegistrationContextFromQuery(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("GET", "/oauth/start?referral_code=AGENT123&invitation_code=INV456", nil)

	context := oauthRegistrationContextFromQuery(c)

	require.Equal(t, "AGENT123", context["referral_code"])
	require.Equal(t, "INV456", context["invitation_code"])
}

func TestOAuthRegistrationContextAcceptsRefAlias(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("GET", "/oauth/start?ref=AGENT123", nil)
...
func TestOAuthRegistrationContextRejectsUnsafeCode(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("GET", "/oauth/start?referral_code=AGENT%0A123&invitation_code="+strings.Repeat("x", 129), nil)

	require.Nil(t, oauthRegistrationContextFromQuery(c))
}
