...
describe('key client setup picker', () => {
 it('shows all 14 icons, reuses existing key scope and enables install-if-missing', async () => {
  ...
  expect(mocks.ticket).toHaveBeenCalledWith(42, expect.objectContaining({ fingerprint: 'fixture-fingerprint' }), { model_ids: ['claude-opus-5'], default_model: 'claude-opus-5' })
 ...
 it('binds only the models the customer kept, and the default must be one of them', async () => {
  const multi = (): ClientSetupPlan => ({ ...plan('claude-code', 'macos'), models: [{ id: 'claude-opus-5', protocol: 'messages' }, { id: 'claude-sonnet-5', protocol: 'messages' }] })
  mocks.plans.mockReset().mockImplementation((_id, os) => Promise.resolve([{ ...multi(), os }]))
  const w = open(); await flushPromises()
  expect(w.findAll('.model-option input')).toHaveLength(2)
  await w.findAll('.model-option input')[0].setValue(false); await flushPromises()
  await w.get('.btn-primary').trigger('click'); await flushPromises()
  expect(mocks.ticket).toHaveBeenCalledWith(42, expect.any(Object), { model_ids: ['claude-sonnet-5'], default_model: 'claude-sonnet-5' })
 })
 ...
 it('discards a previous OS response and requests plans again', async () => {
  ...
  await w.get('.btn-primary').trigger('click'); await flushPromises()
  expect(mocks.ticket).toHaveBeenCalledWith(42, expect.objectContaining({ os: 'windows' }), expect.objectContaining({ default_model: 'claude-opus-5' }))
