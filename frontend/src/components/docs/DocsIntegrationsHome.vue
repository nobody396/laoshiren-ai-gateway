<template>
  <div class="integrations-home">
    ...
    <header class="hero">
      ...
      <h1>工具集成</h1>
      <span>这里只展示已经核对完成的工具教程。尚未完成的新教程和旧版复杂内容暂时隐藏。</span>
    </header>

    <section class="refresh-notice" aria-label="教程整理状态">
      <strong>当前已开放 {{ visibleClientMatrix.length }} 个教程</strong>
      <p>其他工具仍保留在代码和能力矩阵源文件中，没有删除；教程确认无误后再逐个显示。</p>
    </section>
    ...
    <section class="clients" aria-labelledby="clients-title">
      <div class="section-heading clients-heading">
        <div><small>CLIENT GUIDES</small><h2 id="clients-title">已完成的工具教程</h2></div>
      ...
      <div class="client-grid">
        <router-link
          v-for="client in visibleClientMatrix"
          :key="client.slug"
          :to="`/docs/${client.slug}`"
          class="client-card"
          :data-client-status="simpleClientGuideById[client.id] ? 'manual' : legacyIntegrationOverviewVisible ? client.one_click_status : 'paused'"
        >
          ...
          <footer>
            <span :class="simpleClientGuideById[client.id] ? 'status-manual' : legacyIntegrationOverviewVisible ? `status-${client.one_click_status}` : 'status-paused'">{{ statusLabel(client) }}</span>
            <b>查看教程状态</b>
          </footer>
        </router-link>
      </div>
...
const visibleClientMatrix = clientMatrix.filter(client => simpleClientGuideById[client.id])
...
function statusLabel(client: ClientMatrixEntry): string {
  if (simpleClientGuideById[client.id]) return '手动配置'
  if (!legacyIntegrationOverviewVisible) return '教程整理中'
  if (client.one_click_status === 'ready') return '一键导入可用'
  if (client.one_click_status === 'prototype') return '手动配置'
  return '集成未开放'
}
...
.status-paused{background:#fef3c7;color:#92400e}.status-manual{background:#dbeafe;color:#1d4ed8}.refresh-notice{margin:1rem 0 2rem;border:1px solid #fde68a;border-radius:.85rem;background:#fffbeb;padding:1rem;color:#854d0e}.refresh-notice strong{font-size:.86rem}.refresh-notice p{margin:.35rem 0 0;font-size:.76rem;line-height:1.65}
</style>
