  ...
  <div class="inline-flex flex-col gap-0.5 text-xs font-medium">
    ...
    <div v-if="planLabel || privacyBadge" class="inline-flex items-center overflow-hidden rounded-md">
      <span v-if="planLabel" :class="['inline-flex items-center gap-1 px-1.5 py-1', planBadgeClass]">
        ...
        <Icon v-else-if="grokPaidPlan" name="bolt" size="xs" data-testid="grok-plan-icon" />
        <span>{{ planLabel }}</span>
      </span>
      <span
        v-if="privacyBadge"
...
const platformLabel = computed(() => {
  ...
  if (props.platform === 'gpt-image') return 'GPT-Image'
...
const isGrokFreePlan = computed(() =>
  props.platform === 'grok' && ['free', 'basic'].includes(normalizedPlanType.value)
)
const grokPaidPlan = computed(() =>
  props.platform === 'grok' && ['supergrok', 'supergrokheavy'].includes(normalizedPlanType.value)
)
...
const expiresLabel = computed(() => {
  ...
  if (['free', 'basic'].includes(normalizedPlanType.value)) return ''
  try {
