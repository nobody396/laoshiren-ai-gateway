...
export function useGrokOAuth() {
  ...
  const generateAuthUrl = async (
    ...
    try {
      ...
      error.value = extractApiErrorMessage(
        err,
        t("admin.accounts.oauth.grok.failedToGenerateUrl"),
      );
  ...
  const buildExtraInfo = (
    ...
    if (tokenInfo.subscription_tier)
      extra.subscription_tier = tokenInfo.subscription_tier;
    if (tokenInfo.entitlement_status)
      extra.entitlement_status = tokenInfo.entitlement_status;
    return extra;
  };
