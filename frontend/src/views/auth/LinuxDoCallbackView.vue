...
function resolveOAuthErrorMessage(error: string, fallback: string): string {
  switch (error) {
    ...
    case 'state_consumed':
      return t('auth.oauth.stateConsumed')
    case 'invalid_state':
      return t('auth.oauth.invalidState')
    case 'binding_conflict':
      return t('auth.oauth.bindingConflict')
    case 'missing_verifier':
      return t('auth.oauth.missingVerifier')
    case 'provider_error':
      return t('auth.oauth.providerError', { provider: providerLabel.value })
    case 'token_exchange_failed':
      return t('auth.oauth.tokenExchangeFailed', { provider: providerLabel.value })
    case 'userinfo_failed':
      return t('auth.oauth.userInfoFailed', { provider: providerLabel.value })
    case 'login_failed':
      return t('auth.oauth.loginFailed', { provider: providerLabel.value })
    case 'binding_failed':
      return t('auth.oauth.bindingFailed')
    default:
...
onMounted(async () => {
  ...
  if (binding === 'success') {
    const target = sanitizeRedirectPath(params.get('redirect') || '/profile')
    appStore.showSuccess(
      t('auth.oauth.bindingSuccess', {
        provider:
          returnedProvider === 'google'
            ? 'Google'
            : returnedProvider === 'github'
              ? 'GitHub'
              : 'Linux.do'
      })
    )
  ...
  if (error) {
    if (error === 'invitation_required') {
      ...
      if (!pendingOAuthToken.value) {
        errorMessage.value = t('auth.oauth.invalidPendingToken')
    ...
    errorMessage.value = resolveOAuthErrorMessage(error, errorDesc || error)
    appStore.showError(errorMessage.value)
    isProcessing.value = false
    return
  }
