...
type AdminService interface {
	...
	// Also returns totalRecharged (sum of successful top-up order credits).
	...
	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, sortBy, sortOrder string) ([]Account, int64, error)
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if oldRole == RoleAdmin && user.Role != RoleAdmin && s.entClient != nil {
		if _, err := s.entClient.AdminUserRole.Delete().Where(adminuserrole.UserIDEQ(user.ID)).Exec(ctx); err != nil {
			logger.LegacyPrintf("service.admin", "failed to clear rbac roles after admin demotion: user_id=%d err=%v", user.ID, err)
		}
	}
	...
	if concurrencyDiff != 0 {
		if s.accountChangeRepo != nil {
			record := &AccountChangeRecord{
				UserID:     user.ID,
				AssetType:  AccountChangeAssetConcurrency,
				Reason:     AccountChangeReasonAdminAdjustment,
				Delta:      float64(concurrencyDiff),
				SourceType: AccountChangeSourceAdminManual,
				CreatedAt:  time.Now(),
			}
			if err := s.accountChangeRepo.Create(ctx, record); err != nil {
				logger.LegacyPrintf("service.admin", "failed to create concurrency account change record: %v", err)
			}
		}
	}
...
func (s *adminServiceImpl) GetUserBalanceHistory(ctx context.Context, userID int64, page, pageSize int, codeType string) ([]RedeemCode, int64, float64, error) {
	if s.accountChangeRepo != nil {
		params := pagination.PaginationParams{Page: page, PageSize: pageSize}
		records, result, err := s.accountChangeRepo.ListByUserPaginated(ctx, userID, params, AccountChangeRecordListFilters{
			DisplayType: codeType,
		})
		if err != nil {
			return nil, 0, 0, err
		}
		totalRecharged, err := s.accountChangeRepo.SumByUser(ctx, userID, AccountChangeAssetBalance, AccountChangeReasonTopup)
		if err != nil {
			return nil, 0, 0, err
		}
...
func (s *adminServiceImpl) validateAccountPlatformConfig(platform, accountType string, credentials map[string]any) error {
	if strings.TrimSpace(platform) != PlatformGPTImage {
		return nil
	}
	if accountType != AccountTypeUpstream {
		return errors.New("gpt-image accounts must use upstream type")
	}

	baseURL := credentialStringValue(credentials, "base_url")
	if baseURL == "" {
		return errors.New("gpt-image accounts require base_url")
	}
	if _, err := s.validateUpstreamBaseURL(baseURL); err != nil {
		return err
	}
	if credentialStringValue(credentials, "api_key") == "" {
		return errors.New("gpt-image accounts require api_key")
	}
	return nil
}

func (s *adminServiceImpl) validateUpstreamBaseURL(raw string) (string, error) {
...
func credentialStringValue(values map[string]any, key string) string {
	if len(values) == 0 {
		return ""
	}
	raw, ok := values[key]
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.ChatbotEnabled != nil {
		group.ChatbotEnabled = *input.ChatbotEnabled
	}
	if input.FallbackGroupID != nil {
		// 校验降级分组
	...
	if input.MessagesDispatchModelConfig != nil {
		group.MessagesDispatchModelConfig = normalizeOpenAIMessagesDispatchModelConfig(*input.MessagesDispatchModelConfig, group.ID)
	}
...
func (s *adminServiceImpl) ReplaceUserGroup(ctx context.Context, userID, oldGroupID, newGroupID int64) (*ReplaceUserGroupResult, error) {
	...
	if s.authCacheInvalidator != nil {
		if keys, keyErr := s.apiKeyRepo.ListKeysByUserID(ctx, userID); keyErr == nil {
			for _, k := range keys {
...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, sortBy, sortOrder string) ([]Account, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
