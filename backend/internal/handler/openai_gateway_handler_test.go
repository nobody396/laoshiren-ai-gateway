...
func TestOpenAIResponses_RejectsPreviousResponseIDOnHTTP(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, http.StatusBadRequest, w.Code)
	require.Contains(t, w.Body.String(), "previous_response_id is only supported on Responses WebSocket v2")
}
