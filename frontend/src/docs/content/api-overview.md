...
- 日志不得保存完整Key、Authorization Header、提示词、上传文件或Base64图片正文。
...
1. 每个应用使用独立Key，并选择明确的分组和模型范围。
2. 用准备上线的Key完成最小真实请求，不使用另一分组的测试结果代替。
3. 按业务需要分别验证流式、工具调用、结构化输出、图片和文件能力。
4. 为 `429`、超时和 `5xx` 设置有上限的退避；非幂等任务不得直接自动重放。
5. 确认日志能定位Request ID，但不会泄露凭据和业务内容。
6. 保留原Provider配置和回滚步骤，先用小流量切换。

失败时提供北京时间、客户端、模型、端点、Request ID和完整错误，再联系支持。不要发送API Key。
