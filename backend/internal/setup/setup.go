...
func createAdminUser(cfg *SetupConfig) (bool, string, error) {
	...
	if strings.TrimSpace(cfg.Admin.Password) == "" {
		return false, "", fmt.Errorf("admin password is required before creating the first admin user; set ADMIN_PASSWORD or default.admin_password")
	}
	...
	if _, err := db.ExecContext(
		...
		`INSERT INTO admin_user_roles (user_id, role_id)
		 SELECT $1, id FROM admin_roles WHERE name = 'super_admin'
