...
// tokenRefreshTempUnschedExtendThreshold avoids extending a temporary
...
func (s *TokenRefreshService) refreshWithRetry(ctx context.Context, account *Account, refresher TokenRefresher, executor OAuthRefreshExecutor, refreshWindow time.Duration) error {
	...
	if shouldExtendTokenRefreshTempUnsched(account, time.Now()) {
		...
		slog.Debug("token_refresh.temp_unschedulable_extend_skipped",
			"account_id", account.ID,
...
func shouldExtendTokenRefreshTempUnsched(account *Account, now time.Time) bool {
	if account == nil || account.TempUnschedulableUntil == nil {
		return true
	}
	return account.TempUnschedulableUntil.Sub(now) <= tokenRefreshTempUnschedExtendThreshold ||
