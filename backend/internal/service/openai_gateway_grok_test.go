...
func TestForwardGrokResponsesCompactReturnsOpenAICompactionItem(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	body := []byte(`{"model":"grok","input":"compact this conversation","stream":false}`)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body: io.NopCloser(strings.NewReader(`{
			"id":"resp_compact","status":"completed","model":"grok-4.5",
			"output":[
				{"type":"reasoning","encrypted_content":"encrypted-compact-state","summary":[]},
	...
	require.Equal(t, "encrypted-compact-state", gjson.Get(recorder.Body.String(), "output.0.encrypted_content").String())
...
func TestForwardGrokResponsesAPIKeyUsesXAIResponses(t *testing.T) {
	...
	require.Contains(t, recorder.Body.String(), `"created_at":`)
}
...
func TestForwardAsAnthropicForGrokRetriesInvalidEncryptedThinkingSignature(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	body := []byte(`{
		...
		"messages":[
			{"role":"assistant","content":[{"type":"thinking","thinking":"prior plan","signature":"encrypted-prior-state"},{"type":"text","text":"prior answer"}]},
			{"role":"user","content":"continue"}
		]
	}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", bytes.NewReader(body))
	c.Set("api_key", &APIKey{ID: 5451})

	account := healthyGrokOAuthGatewayTestAccount(5451, "access-token")
	repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
		accountsByID: map[int64]*Account{5451: account},
	}}
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		{
			StatusCode: http.StatusBadRequest,
		...
		grokMessagesSSECompletedResponse("resp_grok_messages_recovered", 0),
	}}
	svc := &OpenAIGatewayService{
		httpUpstream:      upstream,
		grokTokenProvider: NewGrokTokenProvider(repo, nil),
	...
	require.NotNil(t, result)
	require.Len(t, upstream.bodies, 2)
	require.Equal(t, "encrypted-prior-state", gjson.GetBytes(upstream.bodies[0], `input.#(type=="reasoning").encrypted_content`).String())
