...
type Group struct {
	...
	// GPT-Image 旧版固定每次调用价格（未配置分辨率价格时兜底）
...
// was not loaded in eager-loading.
func (e GroupEdges) AccountChangeRecordsOrErr() ([]*AccountChangeRecord, error) {
	if e.loadedTypes[2] {
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldRateMultiplier, group.FieldDailyLimitUsd, group.FieldWeeklyLimitUsd, group.FieldMonthlyLimitUsd, group.FieldImageRateMultiplier, group.FieldImagePrice1k, group.FieldImagePrice2k, group.FieldImagePrice4k, group.FieldGptImageCallPrice, group.FieldVideoRateMultiplier, group.FieldVideoPrice480p, group.FieldVideoPrice720p, group.FieldVideoPrice1080p:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldGptImageCallPrice:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field gpt_image_call_price", values[i])
			} else if value.Valid {
				_m.GptImageCallPrice = new(float64)
				*_m.GptImageCallPrice = value.Float64
			}
...
// QueryAccountChangeRecords queries the "account_change_records" edge of the Group entity.
func (_m *Group) QueryAccountChangeRecords() *AccountChangeRecordQuery {
	return NewGroupClient(_m.config).QueryAccountChangeRecords(_m)
}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.GptImageCallPrice; v != nil {
		builder.WriteString("gpt_image_call_price=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
