...
达标用户经过账户状态与风险检查后可立即开通 Agent；系统保存
`qualified_at` 与 `activated_at` 并创建默认动态链接。支付宝主体资料在提现前
必须完成唯一性检查与管理员验证。激活前消费可用于资格判断，但不得追溯生成
现金佣金。
...
- 客户绑定时快照：
  ...
  - `link_rate_version`
  - `bound_at`
- 链接费率降低只影响未来绑定用户；已绑定客户保留原有更高返利。
- 链接费率提高同时提升既有绑定客户，返利只能升不能降。
- 默认链接始终保持启用；活动链接可以暂停。
- 暂停链接不影响已经绑定的客户。
