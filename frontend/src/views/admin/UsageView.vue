...
const modelDistributionSource = ref<'requested' | 'upstream' | 'mapping'>('requested')
...
const loadLogs = async () => {
  ...
  try {
    const res = await adminAPI.usage.list(buildUsageListParams(pagination.page, pagination.page_size, false), { signal: c.signal })
...
const loadChartData = async () => {
  ...
  try {
    ...
    const snapshot = await adminAPI.dashboard.getSnapshotV2({
      ...
      model_source: modelDistributionSource.value,
      include_stats: false,
      include_trend: true,
...
const accountBilled = (log: Partial<AdminUsageLog>): number => {
  ...
  try {
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.inbound_endpoint || '', log.upstream_endpoint || '', getRequestTypeLabel(log), log.billing_mode || 'token',
