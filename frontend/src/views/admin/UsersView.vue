<template>
  <AppLayout>
            ...
            <div v-if="visibleFilters.has('role')" class="w-full sm:w-32">
              <Select
                ...
                :options="[
                  ...
                  { value: 'agent', label: t('admin.users.agent') },
        ...
        <DataTable :columns="columns" :data="users" :loading="loading" :actions-count="7">
          ...
          <template #cell-last_active_at="{ value }">
            <span class="text-sm text-gray-500 dark:text-dark-400">
              {{ value ? formatDateTime(value) : t('common.time.never') }}
            </span>
          </template>
        ...
        <div class="py-1">
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <template v-if="getReplaceableExclusiveGroups(user).length > 0">
                <button
                  v-for="group in getReplaceableExclusiveGroups(user)"
                  :key="`${user.id}-${group.id}`"
                  @click="openGroupReplace(user, group); closeActionMenu()"
                  class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
                >
                  <Icon name="swap" size="sm" class="text-primary-500" :stroke-width="2" />
                  <span class="min-w-0 truncate">
                    {{ `${t('admin.users.replaceGroup')} · ${group.name}` }}
                  </span>
                </button>
              </template>
              ...
              <!-- Set/Remove Agent Role (not for admin) -->
              <button
                v-if="user.role !== 'admin' && user.role !== 'agent'"
                @click="handleSetAgentLevel(user, 'light'); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                ...
                {{ t('admin.users.setAsLightAgent') }}
              </button>
              <button
                v-if="user.role !== 'admin' && user.role !== 'agent'"
                @click="handleSetAgentLevel(user, 'standard'); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <svg class="h-4 w-4 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
                </svg>
                {{ t('admin.users.setAsStandardAgent') }}
              </button>
              <button
                v-if="user.role === 'agent'"
                @click="handleToggleAgent(user); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <svg class="h-4 w-4 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
                </svg>
                {{ t('admin.users.removeAgent') }}
              </button>

              <!-- Promote to Admin (only user/agent) -->
              <button
                v-if="user.role !== 'admin'"
                @click="handlePromoteToAdmin(user); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <Icon name="shield" size="sm" class="text-indigo-500" :stroke-width="2" />
                {{ t('admin.users.promoteToAdmin') }}
              </button>

              <!-- Assign RBAC Roles (only admin) -->
              <button
                v-if="user.role === 'admin'"
                @click="handleAssignRoles(user); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <Icon name="shield" size="sm" class="text-indigo-500" :stroke-width="2" />
              ...
              <!-- Demote Admin (only admin) -->
              <button
                v-if="user.role === 'admin'"
                @click="handleDemoteAdmin(user); closeActionMenu()"
    ...
    <GroupReplaceModal :show="showGroupReplaceModal" :user="groupReplaceUser" :old-group="groupReplaceOldGroup" :all-groups="allGroups" @close="closeGroupReplaceModal" @success="handleGroupReplaceSuccess" />
    <UserAttributesConfigModal :show="showAttributesModal" @close="handleAttributesModalClose" />
    <UserRolesModal :show="showRolesModal" :user="rolesModalUser" @close="closeRolesModal" @success="loadUsers" />
    <ConfirmDialog :show="showDemoteDialog" :title="t('admin.users.demoteAdmin')" :message="t('admin.users.demoteConfirm', { email: demotingUser?.email })" :danger="true" @confirm="confirmDemoteAdmin" @cancel="showDemoteDialog = false" />
    <ConfirmDialog :show="showPromoteDialog" :title="t('admin.users.promoteToAdmin')" :message="t('admin.users.promoteConfirm', { email: promotingUser?.email })" @confirm="confirmPromoteToAdmin" @cancel="showPromoteDialog = false" />
  </AppLayout>
</template>
...
const getReplaceableExclusiveGroups = (user: AdminUser) => {
  if (!user.allowed_groups?.length) return []
  return allGroups.value.filter((group) =>
    group.status === 'active' &&
    group.is_exclusive &&
    group.subscription_type === 'standard' &&
    user.allowed_groups?.includes(group.id)
  )
}
...
const rolesModalUser = ref<AdminUser | null>(null)
const showPromoteDialog = ref(false)
const promotingUser = ref<AdminUser | null>(null)
const showDemoteDialog = ref(false)
const demotingUser = ref<AdminUser | null>(null)
...
const syncSelectedUser = (selected: { value: AdminUser | null }) => {
  if (!selected.value) return
  selected.value = users.value.find((user) => user.id === selected.value?.id) ?? selected.value
}

const syncSelectedUsers = () => {
  syncSelectedUser(editingUser)
  syncSelectedUser(viewingUser)
  syncSelectedUser(allowedGroupsUser)
  syncSelectedUser(balanceUser)
  syncSelectedUser(balanceHistoryUser)
  syncSelectedUser(groupReplaceUser)
}
...
const handleToggleAgent = async (user: AdminUser) => {
  const newRole = user.role === 'agent' ? 'user' : 'agent'
  try {
    await adminAPI.users.update(user.id, { role: newRole as 'user' | 'agent' })
    appStore.showSuccess(
      newRole === 'agent' ? t('admin.users.setAsAgentSuccess') : t('admin.users.removeAgentSuccess')
    )
    loadUsers()
  } catch (error: any) {
    appStore.showError(error.response?.data?.detail || t('admin.users.failedToUpdate'))
    console.error('Error toggling agent role:', error)
  }
}

const handleSetAgentLevel = async (user: AdminUser, level: 'light' | 'standard') => {
  try {
    await adminAPI.users.update(user.id, { role: 'agent', agent_level_key: level })
    appStore.showSuccess(
      level === 'standard' ? t('admin.users.setAsStandardAgentSuccess') : t('admin.users.setAsLightAgentSuccess')
    )
    loadUsers()
  } catch (error: any) {
    appStore.showError(error.response?.data?.detail || t('admin.users.failedToUpdate'))
    console.error('Error setting agent level:', error)
  }
}
...
const confirmPromoteToAdmin = async () => {
  ...
  try {
    ...
    appStore.showSuccess(t('admin.users.promoteSuccess'))
...
const handleDemoteAdmin = (user: AdminUser) => {
  demotingUser.value = user
  showDemoteDialog.value = true
}

const confirmDemoteAdmin = async () => {
  const user = demotingUser.value
  if (!user) return
  try {
    await rbacAPI.setUserRoles(user.id, { role_ids: [] })
    await adminAPI.users.update(user.id, { role: 'user' })
    appStore.showSuccess(t('admin.users.demoteSuccess'))
...
const handleGroupReplaceSuccess = async () => {
  await loadUsers()
}
