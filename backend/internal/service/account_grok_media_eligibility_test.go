...
func TestGrokMediaCapabilityKeepsOnlyUnobservedOAuthAsProbeCandidate(t *testing.T) {
	...
	require.True(t, supportsGrokMediaGenerationScheduling(unobserved))
	...
	require.False(t, supportsGrokMediaGenerationScheduling(inconclusive))
}
...
func TestGrokMediaCapabilityFiltersOnlyGeneration(t *testing.T) {
	...
	require.True(t, grokAccountEligibleForScheduling(account, "", nil, false))
	require.False(t, grokAccountEligibleForScheduling(account, "grok-imagine-video", nil, true))
}
