          ...
          <transition name="dropdown">
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div class="py-1">
                ...
                <a
                  href="https://github.com/bozhouDev/DragonCode-sub2api"
                  target="_blank"
                  rel="noopener noreferrer"
                  @click="closeDropdown"
                  class="dropdown-item"
...
async function handleOpenChatbot(apiKeyId?: number) {
  closeDropdown()
  if (!chatbotUrl.value) return

  try {
    const { ticket } = await issueSSOTicket(apiKeyId)
    const base = chatbotUrl.value.replace(/\/+$/, '')
