...
func (s *openAI403CounterStub) IncrementOpenAI403Count(context.Context, int64, int) (int64, error) {
	s.count++
	return s.count, nil
}

func (s *openAI403CounterStub) ResetOpenAI403Count(context.Context, int64) error {
	s.resetHits++
	s.count = 0
	return nil
}

func TestRateLimitService_HandleUpstreamError_OpenAI403RequiresConsecutiveFailures(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		shouldDisable := svc.HandleUpstreamError(context.Background(), account, 403, http.Header{}, []byte(`{"error":{"message":"forbidden"}}`))
		if !shouldDisable {
			t.Fatalf("attempt %d: expected shouldDisable=true", i+1)
		}
	}

	if repo.tempCalls != 2 {
		t.Fatalf("expected first two OpenAI 403s to set temp unschedulable twice, got %d", repo.tempCalls)
	}
	if repo.setErrorCalls != 0 {
		t.Fatalf("expected no SetError before threshold, got %d", repo.setErrorCalls)
	}

	shouldDisable := svc.HandleUpstreamError(context.Background(), account, 403, http.Header{}, []byte(`{"error":{"message":"forbidden"}}`))
	if !shouldDisable {
		t.Fatal("expected threshold attempt to return shouldDisable=true")
	}
	if repo.setErrorCalls != 1 {
		t.Fatalf("expected third OpenAI 403 to SetError once, got %d", repo.setErrorCalls)
	}
	if !strings.Contains(repo.lastErrorMsg, "consecutive_403=3/3") {
		t.Fatalf("expected error to include consecutive counter, got %q", repo.lastErrorMsg)
	}
}
