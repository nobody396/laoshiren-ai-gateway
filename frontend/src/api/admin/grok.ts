...
export function getGrokSSOImportTimeout(keyCount: number): number {
  const batches = Math.ceil(
    Math.max(1, keyCount) / GROK_SSO_IMPORT_CONCURRENCY,
  );
  return (
    batches * GROK_SSO_IMPORT_TIMEOUT_PER_BATCH_MS +
    GROK_SSO_IMPORT_TIMEOUT_BUFFER_MS
  );
}
