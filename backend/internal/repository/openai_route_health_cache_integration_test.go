...
func (s *OpenAIRouteHealthCacheSuite) TestHalfOpenPermitIsSingleOwnerAndIdempotent() {
	...
	require.NoError(s.T(), err)
	require.False(s.T(), ok)
	ok, err = s.cache.RefreshHalfOpenPermit(s.ctx, s.key, "worker-b")
	require.NoError(s.T(), err)
	require.False(s.T(), ok, "a non-owner cannot create or renew a permit")
	ok, err = s.cache.RefreshHalfOpenPermit(s.ctx, s.key, "worker-a")
	require.NoError(s.T(), err)
	require.True(s.T(), ok)
	...
	ok, err = s.cache.RefreshHalfOpenPermit(s.ctx, s.key, "worker-a")
	require.NoError(s.T(), err)
	require.False(s.T(), ok, "refresh must never reacquire an expired or released lease")
	ok, err = s.cache.AcquireHalfOpenPermit(s.ctx, s.key, "worker-b")
	require.NoError(s.T(), err)
	require.True(s.T(), ok)
}
