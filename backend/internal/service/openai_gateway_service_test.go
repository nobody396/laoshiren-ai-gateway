...
func TestOpenAISelectAccountWithLoadAwarenessStickySpilloverKeepsBinding(t *testing.T) {
	sessionHash := "sticky-spillover"
	groupID := int64(1)
	repo := stubOpenAIAccountRepo{accounts: []Account{
		{ID: 1, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 6, Priority: 1},
		{ID: 2, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 6, Priority: 1},
	}}
...
func TestOpenAIStreamingMissingTerminalEventBeforeOutputReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
...
func TestExtractOpenAIUsageFromJSONBytes_AcceptsResponseAndChatUsageShapes(t *testing.T) {
	...
	usage, ok = extractOpenAIUsageFromJSONBytes([]byte(`{"data":{"usage":{"input_tokens":17,"output_tokens":19,"input_tokens_details":{"cached_tokens":6}}}}`))
	require.True(t, ok)
	require.Equal(t, 17, usage.InputTokens)
	require.Equal(t, 19, usage.OutputTokens)
	require.Equal(t, 6, usage.CacheReadInputTokens)

	usage, ok = extractOpenAIUsageFromJSONBytes([]byte(`{"data":{"response":{"usage":{"prompt_tokens":23,"completion_tokens":29,"prompt_tokens_details":{"cached_tokens":7}}}}}`))
	require.True(t, ok)
	require.Equal(t, 23, usage.InputTokens)
	require.Equal(t, 29, usage.OutputTokens)
	require.Equal(t, 7, usage.CacheReadInputTokens)

	usage, ok = extractOpenAIUsageFromJSONBytes([]byte(`{"usage":{"input_tokens":31,"output_tokens":37},"response":{"usage":{"input_tokens":41,"output_tokens":43}},"data":{"usage":{"input_tokens":47,"output_tokens":53}}}`))
	require.True(t, ok)
...
func TestExtractCodexFinalResponse_EventNamedTerminal(t *testing.T) {
	body := strings.Join([]string{
		`event: response.completed`,
		`data: {"response":{"id":"resp_named","model":"gpt-4o","usage":{"prompt_tokens":11,"completion_tokens":22,"prompt_tokens_details":{"cached_tokens":3}}}}`,
		``,
	}, "\n")

	finalResp, ok := extractCodexFinalResponse(body)
	require.True(t, ok)
	require.Contains(t, string(finalResp), `"id":"resp_named"`)

	usage := (&OpenAIGatewayService{}).parseSSEUsageFromBody(body)
