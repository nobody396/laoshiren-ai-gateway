...
describe('ChangelogHeaderLink', () => {
  ...
  it('links to the changelog and highlights a new published update', async () => {
    ...
    expect(wrapper.get('[data-testid="changelog-header-label"]').classes()).toContain('sm:inline')
    expect(wrapper.get('[data-testid="changelog-header-label"]').classes()).not.toContain('2xl:inline')
