...
const DefaultCSPPolicy = "default-src 'self'; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://static.cloudflareinsights.com https://www.googletagmanager.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' https:; frame-src https://challenges.cloudflare.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
func (c GPTImageS3Config) IsConfigured() bool {
	return c.Enabled && strings.TrimSpace(c.Bucket) != ""
}

type GPTImageTaskSettlementConfig struct {
	Enabled         bool `mapstructure:"enabled"`
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	cfg.GitHub.AuthorizeURL = strings.TrimSpace(cfg.GitHub.AuthorizeURL)
	cfg.GitHub.TokenURL = strings.TrimSpace(cfg.GitHub.TokenURL)
	cfg.GitHub.UserInfoURL = strings.TrimSpace(cfg.GitHub.UserInfoURL)
	cfg.GitHub.EmailsURL = strings.TrimSpace(cfg.GitHub.EmailsURL)
	cfg.GitHub.Scopes = strings.TrimSpace(cfg.GitHub.Scopes)
...
func setDefaults() {
	...
	viper.SetDefault("oidc_connect.provider_name", "Google")
	viper.SetDefault("oidc_connect.client_id", "")
	viper.SetDefault("oidc_connect.client_secret", "")
	viper.SetDefault("oidc_connect.issuer_url", "https://accounts.google.com")
	viper.SetDefault("oidc_connect.discovery_url", "https://accounts.google.com/.well-known/openid-configuration")
	viper.SetDefault("oidc_connect.authorize_url", "https://accounts.google.com/o/oauth2/v2/auth")
	viper.SetDefault("oidc_connect.token_url", "https://oauth2.googleapis.com/token")
	viper.SetDefault("oidc_connect.userinfo_url", "https://openidconnect.googleapis.com/v1/userinfo")
	viper.SetDefault("oidc_connect.jwks_url", "https://www.googleapis.com/oauth2/v3/certs")
	viper.SetDefault("oidc_connect.scopes", "openid profile email")
	viper.SetDefault("oidc_connect.redirect_url", "")
	viper.SetDefault("oidc_connect.frontend_redirect_url", "/auth/google/callback")
	...
	viper.SetDefault("oidc_connect.allowed_signing_algs", "RS256")
	viper.SetDefault("oidc_connect.clock_skew_seconds", 120)
	viper.SetDefault("oidc_connect.require_email_verified", true)
	viper.SetDefault("oidc_connect.userinfo_email_path", "email")
	viper.SetDefault("oidc_connect.userinfo_id_path", "sub")
	viper.SetDefault("oidc_connect.userinfo_username_path", "name")
	...
	viper.SetDefault("gateway.gpt_image_s3.bucket", "")
	viper.SetDefault("gateway.gpt_image_s3.access_key_id", "")
	viper.SetDefault("gateway.gpt_image_s3.secret_access_key", "")
	...
	viper.SetDefault("gateway.openai_ws.sticky_session_ttl_seconds", 1800)
	...
	// Never make silent data loss the default: when the bounded worker queue is
	// full, execute the accounting task synchronously after the response.
