...
func NewAuthHandler(cfg *config.Config, authService *service.AuthService, identityService *service.IdentityService, userService *service.UserService, apiKeyService *service.APIKeyService, settingService *service.SettingService, promoService *service.PromoService, redeemService *service.RedeemService, totpService *service.TotpService) *AuthHandler {
	return &AuthHandler{
		cfg:           cfg,
...
type RegisterRequest struct {
	...
	InvitationCode string `json:"invitation_code"` // 门控注册邀请码（与 ReferralCode 语义不同）
	ReferralCode   string `json:"referral_code"`   // 分佣邀请码（可选，用于绑定邀请关系）
}
...
// IssueSSOTicket creates a short-lived one-time login ticket for trusted external apps.
// POST /api/v1/auth/sso/ticket
func (h *AuthHandler) IssueSSOTicket(c *gin.Context) {
	...
	if req.APIKeyID != nil {
		...
		if key.Group == nil || !key.Group.ChatbotEnabled {
			response.Forbidden(c, "This API key group is not enabled for chatbot")
			return
		}
		if key.Status != service.StatusActive {
			response.Forbidden(c, "API key is not active")
			return
		}
	}

	ticket, ttl, err := h.authService.IssueSSOTicket(c.Request.Context(), subject.UserID, req.APIKeyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	response.Success(c, SSOTicketResponse{
		Ticket:    ticket,
		ExpiresIn: int(ttl.Seconds()),
	})
}

// ExchangeSSOTicket exchanges a one-time SSO ticket for the standard token pair.
// POST /api/v1/auth/sso/exchange
func (h *AuthHandler) ExchangeSSOTicket(c *gin.Context) {
	...
	pair, user, apiKeyID, err := h.authService.ExchangeSSOTicket(c.Request.Context(), req.Ticket)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
