...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	if account.Platform == PlatformGrok {
		// Derive from the original client turn before model/image rewrites and do
		// not trust a client-supplied X-Grok-Conv-Id directly.
	...
	// Grok Composer does not accept image_url parts. Describe those images via
	// Grok Build first, then replace them with textual descriptions.
	var bridgeUsage OpenAIUsage
	if account.Platform == PlatformGrok {
		bridgedBody, usage, bridged, bridgeErr := s.bridgeGrokComposerImageInputs(ctx, c, account, upstreamBody, authToken)
		if bridgeErr != nil {
