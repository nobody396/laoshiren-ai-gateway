...
func (_q *GroupQuery) QueryAccountChangeRecords() *AccountChangeRecordQuery {
...
func (_q *GroupQuery) WithAccountChangeRecords(opts ...func(*AccountChangeRecordQuery)) *GroupQuery {
	query := (&AccountChangeRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAccountChangeRecords; query != nil {
		if err := _q.loadAccountChangeRecords(ctx, query, nodes,
			func(n *Group) { n.Edges.AccountChangeRecords = []*AccountChangeRecord{} },
			func(n *Group, e *AccountChangeRecord) {
...
func (_q *GroupQuery) loadAccountChangeRecords(ctx context.Context, query *AccountChangeRecordQuery, nodes []*Group, init func(*Group), assign func(*Group, *AccountChangeRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountchangerecord.FieldGroupID)
	}
	query.Where(predicate.AccountChangeRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.AccountChangeRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
