...
SELECT
 ...
 COALESCE((SELECT SUM(e.confirmed_amount_micros-COALESCE(r.reversed,0)) FROM monthly_entitlement_consumptions e LEFT JOIN (SELECT consumption_id,SUM(confirmed_amount_micros) reversed FROM monthly_entitlement_consumption_reversals GROUP BY consumption_id) r ON r.consumption_id=e.id WHERE e.user_id=$1 AND e.created_at>=$2 AND e.created_at<$3),0)::bigint,
 COALESCE((SELECT SUM(GREATEST(c.confirmed_consumption_micros-COALESCE(e.recorded,0),0)) FROM monthly_entitlement_cycles c LEFT JOIN (SELECT x.cycle_id,SUM(x.confirmed_amount_micros-COALESCE(r.reversed,0)) recorded FROM monthly_entitlement_consumptions x LEFT JOIN (SELECT consumption_id,SUM(confirmed_amount_micros) reversed FROM monthly_entitlement_consumption_reversals GROUP BY consumption_id) r ON r.consumption_id=x.id GROUP BY x.cycle_id) e ON e.cycle_id=c.id WHERE c.user_id=$1 AND c.source_type IN('paid_redeem','paid_topup') AND c.starts_at<$3 AND c.ends_at>$2),0)::bigint`, userID, start, end).Scan(&result.BalancePaidConsumptionMicros, &result.BuilderPassConsumptionMicros, &result.UnattributedBuilderPassConsumptionMicros)
	if err != nil {
		return result, err
	}
