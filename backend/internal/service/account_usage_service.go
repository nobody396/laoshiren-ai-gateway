...
import (
	...
	httppool "github.com/bozhouDev/DragonCode-sub2api/internal/pkg/httpclient"
	openaipkg "github.com/bozhouDev/DragonCode-sub2api/internal/pkg/openai"
...
type UsageInfo struct {
	...
	// Grok / xAI passive and actively probed quota snapshots.
...
type ClaudeUsageFetchOptions struct {
	...
	EnableTLSFingerprint bool                    // 兼容旧调用方的布尔开关
...
func (s *AccountUsageService) SetGrokQuotaFetcher(fetcher *GrokQuotaFetcher) {
	s.grokQuotaFetcher = fetcher
}
...
func (s *AccountUsageService) getGrokUsage(ctx context.Context, account *Account, force bool) (*UsageInfo, error) {
	...
	if account != nil && account.IsGrokOAuth() && s.grokQuotaService != nil &&
		(force || grokBillingSnapshotNeedsRefresh(account, now)) && s.shouldProbeGrokBilling(account.ID, now, force) {
