<template>
  <div class="integrations-home">
    ...
    <header class="hero">
      ...
      <h1>工具集成</h1>
      <span>从客户端矩阵查看协议、配置基线和开放状态；只有真实闭环完成后才提供导入命令。</span>
    </header>

    <section class="flow" aria-labelledby="integration-flow-title">
    ...
    <section class="clients" aria-labelledby="clients-title">
      <div class="section-heading clients-heading">
        ...
        <div class="status-legend" aria-label="客户端集成状态">
      ...
      <div class="client-grid">
        <router-link
          ...
          :to="`/docs/${client.slug}`"
          class="client-card"
          :data-client-status="client.one_click_status"
        >
          ...
          <footer>
            <span :class="`status-${client.one_click_status}`">{{ statusLabel(client.one_click_status) }}</span>
            <b>查看配置合同</b>
          </footer>
        </router-link>
      </div>
