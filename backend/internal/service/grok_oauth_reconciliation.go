...
func (s *TokenRefreshService) ReconcileGrokOAuth(ctx context.Context, input GrokOAuthReconcileInput) (*GrokOAuthReconcileResult, error) {
	...
	for i := range active {
		account := active[i]
		if account.ID <= input.AfterID || !account.IsGrokOAuth() || account.Status != StatusActive {
			continue
		}
	...
	refresher, executor, ok := s.grokRegistration()
	if !ok {
...
func (s *TokenRefreshService) grokRegistration() (TokenRefresher, OAuthRefreshExecutor, bool) {
	for i, refresher := range s.refreshers {
		...
		if refresher.CanRefresh(probe) {
			var executor OAuthRefreshExecutor
			if i < len(s.executors) {
				executor = s.executors[i]
			}
			return refresher, executor, true
		}
	}
