...
import (
	...
	dbapikey "github.com/bozhouDev/DragonCode-sub2api/ent/apikey"
	dbgroup "github.com/bozhouDev/DragonCode-sub2api/ent/group"
	dbuser "github.com/bozhouDev/DragonCode-sub2api/ent/user"
	dbusersub "github.com/bozhouDev/DragonCode-sub2api/ent/usersubscription"
...
func (r *usageLogRepository) fillDashboardUsageStatsAggregated(ctx context.Context, stats *DashboardStats, todayUTC, now time.Time) error {
	...
	if err := r.fillDashboardYesterdayActualCostAggregated(ctx, stats, todayUTC); err != nil {
		return err
	}
...
func (r *usageLogRepository) fillDashboardYesterdayActualCostAggregated(ctx context.Context, stats *DashboardStats, todayUTC time.Time) error {
	yesterdayStart := todayUTC.AddDate(0, 0, -1)
	yesterdayStatsQuery := `
		SELECT COALESCE(actual_cost, 0) as yesterday_actual_cost
		FROM usage_dashboard_daily
		WHERE bucket_date = $1::date
	`
	if err := scanSingleRow(ctx, r.sql, yesterdayStatsQuery, []any{yesterdayStart}, &stats.YesterdayActualCost); err != nil {
		if err != sql.ErrNoRows {
			return err
		}
	}
...
func (r *usageLogRepository) fillDashboardUsageStatsFromUsageLogs(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, now time.Time) error {
	...
	if err := r.fillDashboardYesterdayActualCostFromUsageLogs(ctx, stats, todayUTC); err != nil {
		return err
	}
...
func (r *usageLogRepository) fillDashboardYesterdayActualCostFromUsageLogs(ctx context.Context, stats *DashboardStats, todayUTC time.Time) error {
	yesterdayStart := todayUTC.AddDate(0, 0, -1)
	yesterdayEnd := todayUTC
	yesterdayStatsQuery := `
		SELECT COALESCE(SUM(actual_cost), 0) as yesterday_actual_cost
		FROM usage_logs
	...
	return scanSingleRow(
		...
		r.sql,
		yesterdayStatsQuery,
		[]any{yesterdayStart, yesterdayEnd},
