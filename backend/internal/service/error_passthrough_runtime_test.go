...
import (
	...
	"github.com/bozhouDev/DragonCode-sub2api/internal/model"
...
func TestGatewayHandleErrorResponse_NoRuleKeepsDefault(t *testing.T) {
	...
	assert.Equal(t, ClientMessageRequestFailed, errField["message"])
}
...
func TestOpenAIHandleErrorResponse_Upstream413StaysNonRetryable413(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	resp := &http.Response{
		...
		Body:       io.NopCloser(bytes.NewReader([]byte("413 Request Entity Too Large"))),
		Header:     http.Header{},
	}
	account := &Account{ID: 12, Platform: PlatformOpenAI, Type: AccountTypeAPIKey}
	...
	require.True(t, ok)
	assert.Equal(t, "invalid_request_error", errField["type"])
	assert.Equal(t, ClientCodeRequestBodyTooLarge, errField["code"])
	assert.Equal(t, ClientMessageRequestBodyTooLarge, errField["message"])
}
...
func TestApplyErrorPassthroughRule_IgnoresPassthroughBodyForClientMessage(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)

	rule := newNonFailoverPassthroughRule(http.StatusUnprocessableEntity, "provider quota", http.StatusBadRequest, "")
	...
	status, errType, errMsg, matched := applyErrorPassthroughRule(
		...
		PlatformOpenAI,
		http.StatusUnprocessableEntity,
		[]byte(`{"error":{"message":"provider quota exceeded for upstream account"}}`),
		http.StatusBadGateway,
		"upstream_error",
		"Upstream request failed",
	)
	...
	assert.Equal(t, ClientMessageRequestFailed, errMsg)
	assert.NotContains(t, errMsg, "provider quota")
}
