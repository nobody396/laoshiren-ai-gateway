...
func (s *SchedulerSnapshotService) pollOutbox() {
	...
	if processedWatermark <= watermark {
	...
	for i := range 3 {
		...
		wmErr = s.cache.SetOutboxWatermark(wmCtx, processedWatermark)
		wmCancel()
		if wmErr == nil {
			break
		}
	...
	if wmErr != nil {
		...
		watermarkForCheck = processedWatermark
	}
...
func (s *SchedulerSnapshotService) rebuildByGroupIDs(ctx context.Context, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	platforms := []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformGrok, PlatformGPTImage, PlatformAntigravity}
	var firstErr error
...
func (s *SchedulerSnapshotService) rebuildBucket(ctx context.Context, bucket SchedulerBucket, reason string) error {
	...
	defer func() {
		...
		defer cancel()
		_ = s.cache.UnlockBucket(unlockCtx, bucket)
	}()
