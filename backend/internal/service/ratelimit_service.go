...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) (shouldDisable bool) {
	...
	case 401:
		...
		if account.Type == AccountTypeOAuth && account.Platform != PlatformAntigravity {
			...
			// Missing refresh_token cannot recover during a cooldown window. Disable it
			// immediately so it is not selected again and fails before reaching upstream.
			if account.Platform == PlatformOpenAI && strings.TrimSpace(account.GetCredential("refresh_token")) == "" {
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	// CDN/proxy HTML pages are endpoint or route failures, not evidence that
	// the account credential is invalid. Keep failover behavior, but do not
	// amplify one bad route into account cooldowns or permanent disablement.
	...
	slog.Warn("openai_403_temp_unschedulable",
...
func parseOpenAIRateLimitResetTime(body []byte) *int64 {
	...
	// 只接受明确的账号用量限制类型；普通错误消息不得驱动长时间冷却。
