...
import (
	...
	errors2 "github.com/bozhouDev/DragonCode-sub2api/internal/pkg/errors"
...
func TestErrorFrom(t *testing.T) {
	gin.SetMode(gin.TestMode)
	entValidationErr := makeEntValidationError(t)
	...
	tests := []struct {
		...
		{
			name:         "ent_validation_error_maps_to_400",
			err:          entValidationErr,
			wantWritten:  true,
			wantHTTPCode: http.StatusBadRequest,
			wantBody: Response{
				Code:     http.StatusBadRequest,
...
func makeEntValidationError(t *testing.T) error {
	t.Helper()
	client := dbent.NewClient()
	_, err := client.InvoiceProfile.Create().
		SetUserID(1).
		SetTitle("").
		SetTaxNumber("TAX001").
	...
	require.Error(t, err)
	return fmt.Errorf("create invoice profile: %w", err)
}
