/**
 * Core type definitions for laoshirenai frontend
 */
...
export interface Group {
  ...
  gpt_image_call_price?: number | null
  // Claude Code 客户端限制
...
export type AccountPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'gpt-image'
...
export interface Account {
  ...
  // 自定义 Base URL（仅 Anthropic OAuth/SetupToken 账号有效）
...
export type TopupOrderPaymentStatus = 'pending' | 'completed' | 'expired'
export type TopupOrderInvoiceStatus = 'none' | 'applied' | 'invoiced'
export type InvoiceRequestStatus = 'pending' | 'exported' | 'completed' | 'rejected'
...
export interface TopupOrderListResponse {
  items: TopupOrderListItem[]
  total: number
  page: number
  page_size: number
  pages: number
  selectable_amount_fen?: number
...
export interface InvoiceRequestOrder {
  ...
  topup_order: TopupOrderListItem
}
...
export interface DashboardStats {
  ...
  total_user_balance: number // 所有用户当前剩余余额总和
  ...
  total_account_cost: number // 累计账号计费
  ...
  today_account_cost: number // 今日账号计费
  yesterday_actual_cost: number // 昨日实际扣除
