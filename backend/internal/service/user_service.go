...
type UserRepository interface {
	...
	TouchLastActive(ctx context.Context, userID int64, ts time.Time) error
	...
	SetInviterAndAgent(ctx context.Context, userID, inviterID int64, agentID *int64) error
	AdminBindUserToAgent(ctx context.Context, userID, agentID int64, setInviterIfEmpty bool) error
	// SetFirstRecharged 原子标记首充完成，返回影响的行数（0 表示已标记过）
	SetFirstRecharged(ctx context.Context, userID int64) (int, error)
	// MarkFirstInvitedTopup 原子标记“被邀请用户首次虎皮椒充值”，返回影响的行数（0 表示非被邀请用户或已标记过）
	MarkFirstInvitedTopup(ctx context.Context, userID, topupOrderID int64) (int, error)
	// GetInviteCodeByUserID 轻量查询用户的 invite_code 字段
	GetInviteCodeByUserID(ctx context.Context, userID int64) (*string, error)
	// CountInvitedByInviterID 统计以 inviterID 为邀请人的用户数（普通用户邀请统计）
	CountInvitedByInviterID(ctx context.Context, inviterID int64) (int64, error)
}
...
func (s *UserService) TouchLastActive(ctx context.Context, userID int64) error {
	...
	return s.userRepo.TouchLastActive(ctx, userID, time.Now())
}
...
func (s *UserService) ChangePassword(ctx context.Context, userID int64, req ChangePasswordRequest) error {
	...
	if _, err := updatePasswordAndIncrementTokenVersion(ctx, s.userRepo, user.ID, user.PasswordHash); err != nil {
