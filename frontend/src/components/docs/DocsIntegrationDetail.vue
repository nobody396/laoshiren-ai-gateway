<template>
  <article class="integration-detail" :data-client-status="client.one_click_status">
    ...
    <header class="detail-hero">
      ...
      <div class="hero-meta">
        <div class="protocol-list" aria-label="原生协议">
        ...
        <span class="status-badge" :class="`status-${client.one_click_status}`">{{ statusLabel }}</span>
      </div>
    </header>

    <section class="availability" :class="`availability-${client.one_click_status}`" aria-label="集成开放状态">
    ...
    <section v-if="client.one_click_status === 'ready'" class="steps">
      <h2>自动配置</h2>
      <ol>
        <li>
          <b>01</b>
    ...
    <section v-else class="automatic-unavailable">
      <h2>自动配置</h2>
      <strong>暂不提供一键命令</strong>
      <p>当前版本还没有通过安全写入和回滚验收，请使用下面的手动配置；页面不会生成占位命令。</p>
    </section>

    <ClaudeCodeManualConfig v-if="isClaude" />
    <ClientManualConfig v-else :client="client" />
    ...
    <router-link to="/docs/models" class="model-link">查看模型能力与客户端证据 →</router-link>
  </article>
</template>
