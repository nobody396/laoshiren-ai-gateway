    ...
    <TablePageLayout>
        ...
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="rounded-lg bg-green-100 p-2 dark:bg-green-900/30">
              <Icon name="bolt" size="md" class="text-green-600 dark:text-green-400" />
            </div>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-image_result_urls="{ row }">
            <a
              v-if="row.image_result_urls?.length"
              :href="row.image_result_urls[0]"
              target="_blank"
              rel="noopener noreferrer"
              class="inline-flex items-center gap-1.5 rounded border border-indigo-200 bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700 transition-colors hover:bg-indigo-100 dark:border-indigo-500/30 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20"
            >
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <template v-if="!tooltipData?.billing_mode || tooltipData.billing_mode === 'token'">
              <div v-if="tooltipData && tooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatTokenPricePerMillion(tooltipData.input_cost, tooltipData.input_tokens, { currencySymbol: '⚡' }) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && tooltipData.output_tokens > 0" class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.outputTokenPrice') }}</span>
                <span class="font-medium text-violet-300">{{ formatTokenPricePerMillion(tooltipData.output_cost, tooltipData.output_tokens, { currencySymbol: '⚡' }) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            </template>
...
const columns = computed<Column[]>(() => [
  ...
  { key: 'image_result_urls', label: t('usage.result'), sortable: false }
])
