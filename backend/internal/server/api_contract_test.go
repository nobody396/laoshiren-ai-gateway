...
import (
	...
	adminhandler "github.com/bozhouDev/DragonCode-sub2api/internal/handler/admin"
...
func newContractDeps(t *testing.T) *contractDeps {
	...
	redeemService := service.NewRedeemService(redeemRepo, nil, userRepo, subscriptionService, nil, nil, nil, nil, nil, nil, nil, nil)
	...
	authHandler := handler.NewAuthHandler(cfg, nil, nil, userService, nil, settingService, nil, redeemService, nil)
...
func requireJSONValueSubset(t *testing.T, expected, actual any, path string) {
	...
	case map[string]any:
		...
		for key, expectedValue := range exp {
			...
			require.Truef(t, exists, "%s.%s missing from actual JSON", path, key)
			requireJSONValueSubset(t, expectedValue, actualValue, path+"."+key)
		}
	case []any:
		act, ok := actual.([]any)
		require.Truef(t, ok, "%s: expected array, got %T", path, actual)
		require.Lenf(t, act, len(exp), "%s: array length mismatch", path)
		for i := range exp {
			requireJSONValueSubset(t, exp[i], act[i], fmt.Sprintf("%s[%d]", path, i))
		}
	default:
		require.Equalf(t, expected, actual, "%s mismatch", path)
	}
}
...
func (stubRedeemCodeRepo) SumGiftedRedeemValue(ctx context.Context) (float64, error) {
	return 0, errors.New("not implemented")
}
