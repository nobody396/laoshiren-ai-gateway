...
func (r *affiliateProgramRepository) UpdateSettings(
	...
	err := scanSingleRow(ctx, r.db, `
		...
		SET
			...
			first_paid_bonus_threshold_micros = $4,
			first_paid_bonus_micros = $5,
			agent_pool_rate_bps = $6,
			qualification_direct_user_count = $7,
			qualification_min_user_consumption_micros = $8,
			qualification_direct_team_consumption_micros = $9,
			qualification_combined_consumption_micros = $10,
			max_campaign_links = $11,
			commission_conversion_multiplier_millis = $12,
			withdrawal_min_micros = $13,
			withdrawal_sla_hours = $14,
			margin_floor_bps = $15,
			updated_by = $16,
			revision = revision + 1,
			updated_at = NOW()
