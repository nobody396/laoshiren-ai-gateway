...
func (r *apiKeyRepository) GetByID(ctx context.Context, id int64) (*service.APIKey, error) {
	...
	outKeys := []service.APIKey{*out}
	r.applyAPIKeyGroupChatbotEnabled(ctx, outKeys)
	out = &outKeys[0]
	return out, nil
}
...
func userEntityToService(u *dbent.User) *service.User {
	...
	out := &service.User{
		...
		AgentID:             u.AgentID,
		FirstRecharged:      u.FirstRecharged,
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	return &service.Group{
		...
		GPTImageCallPrice:               g.GptImageCallPrice,
...
func (r *apiKeyRepository) applyAPIKeyGroupChatbotEnabled(ctx context.Context, keys []service.APIKey) {
	groupIDs := make([]int64, 0, len(keys))
	for i := range keys {
		if keys[i].Group != nil {
			groupIDs = append(groupIDs, keys[i].Group.ID)
		}
	}
	if len(groupIDs) == 0 {
		return
	...
	flags := make(map[int64]bool, len(groupIDs))
	for rows.Next() {
		var id int64
		var enabled bool
	...
	for i := range keys {
		if keys[i].Group != nil {
			keys[i].Group.ChatbotEnabled = flags[keys[i].Group.ID]
		}
	}
}
