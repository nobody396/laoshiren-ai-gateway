<template>
  <div class="card p-4">
    <div class="mb-4 flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
      <h3 class="shrink-0 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-white">
        {{ title || t('usage.endpointDistribution') }}
      </h3>
      <div class="flex w-full flex-col gap-2 xl:w-auto xl:items-end">
        <div
          v-if="showSourceToggle"
          class="inline-flex max-w-full flex-wrap rounded-lg border border-gray-200 bg-gray-50 p-0.5 dark:border-gray-700 dark:bg-dark-800"
        >
          <button
            type="button"
    ...
    <div v-else-if="displayEndpointStats.length > 0 && chartData" class="flex flex-col gap-6 xl:flex-row xl:items-center">
      <div class="mx-auto h-48 w-48 shrink-0 xl:mx-0">
        <Doughnut :data="chartData" :options="doughnutOptions" />
      </div>
      <div class="max-h-64 min-w-0 flex-1 overflow-x-auto overflow-y-auto">
...
const toggleBreakdown = async (endpoint: string) => {
  ...
  try {
    ...
    breakdownItems.value = response.users || []
  } catch {
    breakdownItems.value = []
  } finally {
