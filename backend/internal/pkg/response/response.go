...
// Accepted returns a 202 response for accepted async operations.
...
func ErrorFrom(c *gin.Context, err error) bool {
	...
	statusCode, status := statusFromError(err)
...
func statusFromError(err error) (int, infraerrors.Status) {
	if err == nil {
		return infraerrors.ToHTTP(nil)
	}
	if appErr := new(infraerrors.ApplicationError); stderrors.As(err, &appErr) {
		return infraerrors.ToHTTP(err)
	}
	if validationErr := new(dbent.ValidationError); stderrors.As(err, &validationErr) {
		return infraerrors.ToHTTP(infraerrors.BadRequest("VALIDATION_ERROR", "invalid request").
			WithMetadata(map[string]string{"field": validationErr.Name}).
			WithCause(err))
	}
	return infraerrors.ToHTTP(err)
}
