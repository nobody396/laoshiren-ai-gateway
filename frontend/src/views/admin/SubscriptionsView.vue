<template>
  <AppLayout>
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <div class="ml-auto flex flex-wrap items-center justify-end gap-3">
            ...
            <button
              ...
              class="btn btn-secondary"
              :title="t('admin.subscriptions.usageGuide')"
            >
              <Icon name="book" size="md" class="mr-2" />
              {{ t('admin.subscriptions.usageGuide') }}
            </button>
    ...
    <BaseDialog
      :show="showGuideModal"
      :title="t('admin.subscriptions.guideTitle')"
      width="normal"
      @close="showGuideModal = false"
    >
      <div class="space-y-4 text-sm text-gray-600 dark:text-gray-300">
        <p>{{ t('admin.subscriptions.guideIntro') }}</p>
        <div class="rounded-xl border border-blue-100 bg-blue-50/70 p-4 dark:border-blue-900/40 dark:bg-blue-900/10">
          <div class="mb-2 text-sm font-semibold text-blue-900 dark:text-blue-200">
            1. {{ t('admin.subscriptions.guideCreateGroupTitle') }}
          </div>
          <p class="leading-6 text-blue-900/80 dark:text-blue-100/80">
            {{ t('admin.subscriptions.guideCreateGroupBody') }}
          </p>
        </div>
        <div class="rounded-xl border border-emerald-100 bg-emerald-50/70 p-4 dark:border-emerald-900/40 dark:bg-emerald-900/10">
          <div class="mb-2 text-sm font-semibold text-emerald-900 dark:text-emerald-200">
            2. {{ t('admin.subscriptions.guideAssignTitle') }}
          </div>
          <p class="leading-6 text-emerald-900/80 dark:text-emerald-100/80">
            {{ t('admin.subscriptions.guideAssignBody') }}
          </p>
        </div>
        <div class="rounded-xl border border-amber-100 bg-amber-50/70 p-4 dark:border-amber-900/40 dark:bg-amber-900/10">
          <div class="mb-2 text-sm font-semibold text-amber-900 dark:text-amber-200">
            3. {{ t('admin.subscriptions.guideOperateTitle') }}
          </div>
          <p class="leading-6 text-amber-900/80 dark:text-amber-100/80">
            {{ t('admin.subscriptions.guideOperateBody') }}
          </p>
        </div>
      </div>
      <template #footer>
        <div class="flex justify-end">
          <button class="btn btn-primary" @click="showGuideModal = false">
            {{ t('common.close') }}
          </button>
        </div>
      </template>
