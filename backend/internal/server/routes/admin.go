...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.RequireAPIPermission())
	{
		...
		// 代理商管理
		registerAgentManagementRoutes(admin, h)
		...
		// Grok / xAI OAuth and quota operations
...
func registerRBACRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	rbac := admin.Group("/rbac")
	{
		...
		rbac.POST("/menus", h.Admin.RBAC.CreateMenu)
		rbac.PUT("/menus/:id", h.Admin.RBAC.UpdateMenu)
		rbac.DELETE("/menus/:id", h.Admin.RBAC.DeleteMenu)

		rbac.GET("/apis", h.Admin.RBAC.ListAPIs)
		rbac.GET("/apis/all", h.Admin.RBAC.ListAllAPIs)
		rbac.GET("/apis/groups", h.Admin.RBAC.GetAPIGroups)
		rbac.POST("/apis", h.Admin.RBAC.CreateAPI)
		rbac.PUT("/apis/:id", h.Admin.RBAC.UpdateAPI)
		rbac.DELETE("/apis/:id", h.Admin.RBAC.DeleteAPI)
		rbac.DELETE("/apis", h.Admin.RBAC.DeleteAPIs)
		rbac.POST("/apis/sync", h.Admin.RBAC.SyncAPIs)

		rbac.GET("/roles", h.Admin.RBAC.ListRoles)
		rbac.POST("/roles", h.Admin.RBAC.CreateRole)
		rbac.PUT("/roles/:id", h.Admin.RBAC.UpdateRole)
		rbac.DELETE("/roles/:id", h.Admin.RBAC.DeleteRole)

		rbac.GET("/roles/:id/menus", h.Admin.RBAC.GetRoleMenus)
		rbac.PUT("/roles/:id/menus", h.Admin.RBAC.SetRoleMenus)
		rbac.GET("/roles/:id/apis", h.Admin.RBAC.GetRoleAPIs)
		rbac.PUT("/roles/:id/apis", h.Admin.RBAC.SetRoleAPIs)

		rbac.GET("/users/:id/roles", h.Admin.RBAC.GetUserRoles)
		rbac.PUT("/users/:id/roles", h.Admin.RBAC.SetUserRoles)
	}
}
...
func registerAgentManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	agents := admin.Group("/agents")
	{
		...
		agents.GET("/settlement-settings", h.Admin.Agent.GetSettlementSettings)
		agents.PUT("/settlement-settings", h.Admin.Agent.UpdateSettlementSettings)
		agents.GET("/affiliate-program", h.Admin.Agent.GetAffiliateProgram)
		agents.PUT("/affiliate-program", h.Admin.Agent.UpdateAffiliateProgram)
		agents.GET("/affiliate-commercial-policy", h.Admin.Agent.GetAffiliateCommercialPolicy)
		agents.GET("/affiliate-qualified-candidates", h.Admin.Agent.ListAffiliateQualifiedCandidates)
		agents.GET("/affiliate-operations-summary", h.Admin.Agent.GetAffiliateOperationsSummary)
		agents.GET("/affiliate-performance", h.Admin.Agent.ListAffiliatePartnerPerformance)
		agents.GET("/affiliate-applications", h.Admin.Agent.ListAffiliateApplications)
		agents.POST("/affiliate-applications/:application_id/review", h.Admin.Agent.ReviewAffiliateApplication)
		agents.GET("/affiliate-risk", h.Admin.Agent.ListAffiliateRiskPrincipals)
		agents.POST("/affiliate-reversals", h.Admin.Agent.ReverseAffiliatePerformance)
		agents.GET("/payment-profiles/pending", h.Admin.Agent.ListPendingPaymentProfiles)
		agents.GET("/affiliate-community", h.Admin.Agent.GetAffiliateCommunity)
		agents.PUT("/affiliate-community", h.Admin.Agent.UpdateAffiliateCommunity)
		agents.POST("/affiliate-community/qr", h.Admin.Agent.UploadAffiliateCommunityQRCode)
		agents.GET("/affiliate-community/qr", h.Admin.Agent.GetAffiliateCommunityQRCode)
		agents.GET("/affiliate-withdrawals", h.Admin.Agent.ListAffiliateWithdrawals)
		agents.POST("/affiliate-withdrawals/:withdrawal_id/complete", h.Admin.Agent.CompleteAffiliateWithdrawal)
		agents.POST("/affiliate-withdrawals/:withdrawal_id/fail", h.Admin.Agent.FailAffiliateWithdrawal)
		agents.GET("/affiliate-withdrawals/:withdrawal_id/payment-qr", h.Admin.Agent.GetAffiliateWithdrawalQRCode)
		agents.GET("/settlement-candidates", h.Admin.Agent.ListSettlementCandidates)
		...
		agents.GET("/:id/commissions", h.Admin.Agent.ListCommissions)
		agents.GET("/:id/settlements", h.Admin.Agent.ListSettlements)
		agents.POST("/:id/settlements", h.Admin.Agent.CreateSettlement)
		agents.GET("/:id/payment-profile", h.Admin.Agent.GetPaymentProfile)
		agents.GET("/:id/payment-profile/alipay-qr", h.Admin.Agent.GetPaymentQRCode)
		agents.PUT("/:id/payment-profile/verification", h.Admin.Agent.ReviewPaymentProfile)
		agents.PUT("/:id/affiliate-risk", h.Admin.Agent.UpdateAffiliateRisk)
		agents.PUT("/:id/self-commission-policy", h.Admin.Agent.UpdateAffiliateSelfCommissionPolicy)
		agents.POST("/:id/affiliate-wallet/purchases", h.Admin.Agent.PurchaseWithAffiliateCommission)
		agents.POST("/:id/affiliate-wallet/purchases/:purchase_id/refund", h.Admin.Agent.RefundAffiliateCommissionPurchase)
		agents.GET("/:id/affiliate-performance", h.Admin.Agent.GetAffiliatePartnerPerformance)
...
func registerOpsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	ops := admin.Group("/ops")
	{
		...
		if h.Status != nil {
			...
			ops.GET("/channel-monitoring", h.Status.GetChannelMonitoring)
		...
		ops.POST("/webhook-notification/test", h.Admin.Ops.TestWebhookNotification)
		...
		// Durable OpenAI shadow-routing decision evidence
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/:id/models/upstream-drift", h.Admin.Account.InspectOpenAIModelDrift)
...
func registerFeedbackRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	feedbacks := admin.Group("/feedbacks")
	{
		...
		feedbacks.GET("/agent-queue", h.Admin.Feedback.AgentQueue)
		feedbacks.GET("/rewards", h.Admin.Feedback.ListRewards)
		feedbacks.POST("/accept", h.Admin.Feedback.AcceptBatch)
		feedbacks.GET("/:id", h.Admin.Feedback.GetByID)
		feedbacks.POST("/:id/agent-analysis", h.Admin.Feedback.AgentTriage)
		feedbacks.POST("/:id/mark-fixing", h.Admin.Feedback.MarkFixing)
		feedbacks.POST("/:id/complete", h.Admin.Feedback.Complete)
		feedbacks.POST("/:id/record-notification", h.Admin.Feedback.RecordNotification)
		feedbacks.POST("/:id/replies", h.Admin.Feedback.CreateReply)
		feedbacks.PUT("/:id/status", h.Admin.Feedback.UpdateStatus)
		feedbacks.PUT("/:id/priority", h.Admin.Feedback.UpdatePriority)
...
func registerRedeemCodeRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	codes := admin.Group("/redeem-codes")
	{
		...
		codes.GET("/billing", h.Admin.Redeem.ListBilling)
		codes.GET("/classification-anomalies", h.Admin.Redeem.ListClassificationAnomalies)
...
func registerInvoiceRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	admin.GET("/topup/orders", h.Admin.Invoice.ListTopupOrders)

	invoice := admin.Group("/invoice")
	{
		invoice.GET("/requests", h.Admin.Invoice.ListRequests)
		invoice.POST("/requests/export", h.Admin.Invoice.ExportRequests)
		invoice.POST("/requests/:id/complete", h.Admin.Invoice.CompleteRequest)
		invoice.POST("/requests/:id/reject", h.Admin.Invoice.RejectRequest)
	}
}
