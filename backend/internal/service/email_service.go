...
func (s *EmailService) SendFeedbackNewEmail(ctx context.Context, email, siteName, title, category, priority, username, userEmail, feedbackURL string) error {
	subject := fmt.Sprintf("[%s] 新用户反馈：%s", siteName, title)
	return s.SendEmail(ctx, email, subject, s.buildFeedbackNewEmailBody(siteName, title, category, priority, username, userEmail, feedbackURL))
}

// SendFeedbackReplyEmail sends a feedback reply notification to end users.
func (s *EmailService) SendFeedbackReplyEmail(ctx context.Context, email, siteName, title, summary, feedbackURL string) error {
	subject := fmt.Sprintf("[%s] 反馈有新回复：%s", siteName, title)
	return s.SendEmail(ctx, email, subject, s.buildFeedbackReplyEmailBody(siteName, title, summary, feedbackURL))
}

// SendBalanceAlert sends a low-balance alert email.
func (s *EmailService) SendBalanceAlert(ctx context.Context, to, siteName, username, balance, threshold, topUpURL string) error {
	subject := fmt.Sprintf("[%s] 余额不足预警", siteName)
	body := s.buildBalanceAlertEmailBody(siteName, username, balance, threshold, topUpURL)
	return s.SendEmail(ctx, to, subject, body)
}

func (s *EmailService) buildFeedbackNewEmailBody(siteName, title, category, priority, username, userEmail, feedbackURL string) string {
	return fmt.Sprintf(`
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"></head>
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background:#f5f7fb; margin:0; padding:24px;">
  <div style="max-width:640px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px; box-shadow:0 8px 24px rgba(15,23,42,.08);">
    <h2 style="margin:0 0 16px; color:#111827;">%s 收到新的用户反馈</h2>
    <p style="margin:0 0 20px; color:#374151;">标题：<strong>%s</strong></p>
    <table style="width:100%%; border-collapse:collapse; margin-bottom:20px;">
      <tr><td style="padding:8px 0; color:#6b7280;">分类</td><td style="padding:8px 0; color:#111827;">%s</td></tr>
      <tr><td style="padding:8px 0; color:#6b7280;">优先级</td><td style="padding:8px 0; color:#111827;">%s</td></tr>
      <tr><td style="padding:8px 0; color:#6b7280;">用户</td><td style="padding:8px 0; color:#111827;">%s</td></tr>
      <tr><td style="padding:8px 0; color:#6b7280;">邮箱</td><td style="padding:8px 0; color:#111827;">%s</td></tr>
    </table>
    <a href="%s" style="display:inline-block; padding:12px 20px; border-radius:10px; background:#2563eb; color:#ffffff; text-decoration:none;">查看反馈</a>
  </div>
</body>
</html>
`, siteName, title, category, priority, username, userEmail, feedbackURL)
}

func (s *EmailService) buildFeedbackReplyEmailBody(siteName, title, summary, feedbackURL string) string {
	return fmt.Sprintf(`
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"></head>
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background:#f5f7fb; margin:0; padding:24px;">
  <div style="max-width:640px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px; box-shadow:0 8px 24px rgba(15,23,42,.08);">
    <h2 style="margin:0 0 16px; color:#111827;">%s 的反馈有新回复</h2>
    <p style="margin:0 0 16px; color:#374151;">反馈标题：<strong>%s</strong></p>
    <div style="padding:16px; border-radius:10px; background:#f9fafb; color:#4b5563; margin-bottom:20px;">%s</div>
    <a href="%s" style="display:inline-block; padding:12px 20px; border-radius:10px; background:#2563eb; color:#ffffff; text-decoration:none;">查看反馈详情</a>
  </div>
</body>
</html>
`, siteName, title, summary, feedbackURL)
}

func (s *EmailService) buildBalanceAlertEmailBody(siteName, username, balance, threshold, topUpURL string) string {
	topUpSection := ""
	if topUpURL != "" {
		topUpSection = fmt.Sprintf(`<a href="%s" class="button">立即充值</a>`, topUpURL)
	}
...
<head>
    ...
    <style>
        ...
        .header h1 { margin: 0; font-size: 24px; }
        .content { padding: 40px 30px; text-align: center; }
        .balance-info { background-color: #fef3c7; border: 1px solid #f59e0b; border-radius: 8px; padding: 20px; margin: 20px 0; }
        .balance-amount { font-size: 32px; font-weight: bold; color: #d97706; }
        .threshold { color: #666; font-size: 14px; margin-top: 8px; }
        .button { display: inline-block; background: linear-gradient(135deg, #f59e0b 0%%, #d97706 100%%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-size: 16px; font-weight: 600; margin: 20px 0; }
        .info { color: #666; font-size: 14px; line-height: 1.6; margin-top: 20px; }
        .footer { background-color: #f8f9fa; padding: 20px; text-align: center; color: #999; font-size: 12px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="content">
            <p style="font-size: 16px; color: #333;">您好, %s</p>
            <p style="color: #666;">您的账户余额已低于预警阈值, 请及时充值以避免 API 服务中断.</p>
            <div class="balance-info">
                <div class="balance-amount">$%s</div>
                <div class="threshold">预警阈值: $%s</div>
            </div>
            %s
            <div class="info">
                <p>余额耗尽后, API 请求将无法处理.</p>
                <p>如您已充值, 请忽略此邮件.</p>
            </div>
        </div>
        <div class="footer">
