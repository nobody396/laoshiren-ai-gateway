...
> 这是我个人沉淀的一套 Claude code和 codex全局规则，放在项目根目录的 `AGENTS.md`或者 `CLAUDE.md` 中，对初学者来说是一个非常省事的规则。

---

## 设计思想：渐进式 Spec

这套规则的核心是**渐进式 Spec（Progressive Specification）**——不同复杂度的需求走不同深度的流程，避免对简单任务过度设计，同时对复杂任务建立足够的约束。
...
每个阶段的自由度是明确的：调研可以自由探索，方案设计可以充分发挥，但**执行阶段自由度为零**，严格按计划来，有偏差立即停下来问。
