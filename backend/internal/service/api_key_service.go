...
func validateCreateAPIKeyRequest(req CreateAPIKeyRequest) error {
	if err := validateAPIKeyGroupIDs(req.GroupID, req.GroupIDs); err != nil {
		return err
	}
...
type groupCacheStatsReader interface {
	GetGroupCacheStats(ctx context.Context, groupIDs []int64, startTime time.Time) (map[int64]GroupCacheStats, error)
}
...
func (s *APIKeyService) Create(ctx context.Context, userID int64, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	if err := s.validateMultiGroupAccess(ctx, user, req.GroupID, req.GroupIDs); err != nil {
		return nil, err
	}
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if req.GroupIDs != nil {
		if err := s.validateMultiGroupAccess(ctx, apiKey.User, req.GroupID, req.GroupIDs); err != nil {
			return nil, err
		}
...
func (s *APIKeyService) GetAvailableGroupCacheStats(ctx context.Context, userID int64, windowDays int) (map[int64]GroupCacheStats, error) {
	...
	for i := range availableGroups {
		if availableGroups[i].ID > 0 {
			groupIDs = append(groupIDs, availableGroups[i].ID)
		}
	}
	if len(groupIDs) == 0 {
		return map[int64]GroupCacheStats{}, nil
	}

	reader, ok := s.groupRepo.(groupCacheStatsReader)
	if !ok {
		return map[int64]GroupCacheStats{}, nil
	}

	startTime := time.Now().Add(-time.Duration(windowDays) * 24 * time.Hour)
	return reader.GetGroupCacheStats(ctx, groupIDs, startTime)
}
