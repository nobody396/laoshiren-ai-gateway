...
// Google Gemini
const geminiModels = [
  ...catalogModelsForPlatform('gemini'),
...
const geminiPresetMappings = [
  ...catalogPresetMappingsForPlatform('gemini'),
...
export function buildModelMappingObject(
  ...
  if (mode === 'whitelist') {
    for (const model of allowedModels) {
      ...
      // 写入 model_mapping 会导致 GetMappedModel() 把真实模型映射成 "claude-*"，从而请求处理失败。
      // 因此这里跳过包含通配符的条目。
      if (!model.includes('*')) {
        mapping[model] = model
      }
