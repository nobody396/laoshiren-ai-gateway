...
func TestCustomerTierExactPaidSourcesConsumptionGraceOverrideAndIncidentSnapshot(t *testing.T) {
	...
	seedCustomerTierPaidEvidence(t, user.ID, prefix, now)
	...
	require.Equal(t, int64(25_000), snapPaid)
...
func TestCustomerTierRefundsAreStructuredPartialFullAndIdempotent(t *testing.T) {
	...
	seedCustomerTierPaidEvidence(t, nativeUser.ID, "native-refund-"+uuid.NewString(), now)
	...
	require.NoError(t, err)
	require.Equal(t, int64(18_000), nativeEvaluation.VerifiedPaidValueCNYFen)
...
func seedCustomerTierPaidEvidence(t *testing.T, userID int64, prefix string, at time.Time) {
	t.Helper()
	ctx := context.Background()
	t.Cleanup(func() {
