...
// AdaptResponsesClientToolsWithInheritedMapping lowers follow-up history when
// a WS turn omits the session-level tools declaration. An explicit tools field
// always replaces the inherited declaration, including an empty one.
...
func retypeResponsesToolCallItemIDMap(item map[string]any, itemType string) {
	id := strings.TrimSpace(stringValue(item["id"]))
	...
	case map[string]any:
		if strings.TrimSpace(stringValue(typed["type"])) == "function_call" {
			...
			if adapter.CustomTools[name] {
				...
				retypeResponsesToolCallItemIDMap(typed, "custom_tool_call")
				...
				retypeResponsesToolCallItemIDMap(typed, "tool_search_call")
...
func (r *ResponsesClientToolStreamRestorer) recordItem(event ResponsesStreamEvent) *responsesClientToolStreamCall {
	...
	if call == nil {
		...
		call = &responsesClientToolStreamCall{
			...
			clientItemID: retypedResponsesToolCallItemID(event.Item.ID, itemType),
