...
// Query params:
//   - type: filter by record type (balance, topup, admin_balance, concurrency, admin_concurrency, subscription)
...
func (h *UserHandler) ReplaceGroup(c *gin.Context) {
	...
	response.Success(c, gin.H{"migrated_keys": result.MigratedKeys})
}
