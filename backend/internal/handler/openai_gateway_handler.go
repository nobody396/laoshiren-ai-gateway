...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	userReleaseFunc, acquired := h.acquireResponsesUserSlot(c, subject.UserID, budgetDecision, reqStream, &streamStarted, reqLog)
	if !acquired {
		return
	}
	...
	for {
		...
		if err != nil {
			...
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, false, nil)
			if reqStream && c.Writer.Status() < 400 {
				markOpsFinalError(c, http.StatusBadGateway, "upstream_error", "OpenAI upstream response failed before completion")
			}
		...
		if err != nil {
			...
			if len(failedAccountIDs) == 0 {
				...
				if lastFailoverErr != nil {
					...
					h.anthropicStreamingAwareError(c, safeErr.StatusCode, safeErr.Type, safeErr.Message, streamStarted)
				}
				return
			}
		}
...
func (h *OpenAIGatewayHandler) acquireResponsesUserSlot(
	c *gin.Context,
	userID int64,
	budgetDecision service.BudgetGuardConcurrencyDecision,
	reqStream bool,
	streamStarted *bool,
	reqLog *zap.Logger,
) (func(), bool) {
	ctx := c.Request.Context()
	userConcurrency := budgetDecision.EffectiveConcurrency
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	subscription, _ := middleware2.GetSubscriptionFromContext(c)
	budgetDecision, err := resolveBudgetGuardConcurrency(ctx, h.billingCacheService, apiKey, subscription, subject.Concurrency)
	if err != nil {
		reqLog.Info("openai.websocket_budget_guard_failed", zap.Error(err))
		closeOpenAIClientWS(wsConn, coderws.StatusPolicyViolation, "billing check failed")
		return
	}
	userReleaseFunc, userAcquired, err := h.concurrencyHelper.TryAcquireUserSlot(ctx, subject.UserID, budgetDecision.EffectiveConcurrency)
	if err != nil {
	...
	hooks := &service.OpenAIWSIngressHooks{
		...
		BeforeTurn: func(turn int) error {
			...
			turnBudgetDecision, err := resolveBudgetGuardConcurrency(ctx, h.billingCacheService, apiKey, subscription, subject.Concurrency)
			if err != nil {
				return service.NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "billing check failed", err)
			}
			userReleaseFunc, userAcquired, err := h.concurrencyHelper.TryAcquireUserSlot(ctx, subject.UserID, turnBudgetDecision.EffectiveConcurrency)
			if err != nil {
...
func (h *OpenAIGatewayHandler) ensureResponsesDependencies(c *gin.Context, reqLog *zap.Logger) bool {
	...
	if c != nil && c.Writer != nil && !c.Writer.Written() {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"error": service.ClientErrorObject(c, "api_error", service.ClientMessageServiceUnavailable),
		})
	}
	return false
}
...
func (h *OpenAIGatewayHandler) submitUsageRecordTask(requestCtx context.Context, task service.UsageRecordTask) {
	submitUsageRecordTaskFailClosed(
		h.usageRecordWorkerPool,
		withUsageRecordRequestCorrelation(requestCtx, task),
		"handler.openai_gateway.responses",
...
func (h *OpenAIGatewayHandler) handleStreamingAwareError(c *gin.Context, status int, errType, message string, streamStarted bool) {
	if (streamStarted || c.Writer.Written()) && getOpsFinalError(c) == nil {
		markOpsFinalError(c, status, errType, message)
	}
	if streamStarted {
		...
		flusher, ok := c.Writer.(http.Flusher)
		if ok {
			errPayload, _ := json.Marshal(service.OpenAIClientErrorEnvelope(c, errType, message))
			if _, err := fmt.Fprintf(c.Writer, "event: error\ndata: %s\n\n", errPayload); err != nil {
				_ = c.Error(err)
			}
			flusher.Flush()
