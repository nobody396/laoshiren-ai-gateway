          ...
          <template #cell-account_count="{ row, value }">
            <div class="space-y-1">
              ...
              <div class="flex flex-wrap gap-1 text-[11px]">
                <span class="rounded bg-emerald-50 px-2 py-0.5 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300">
                  {{
                    t('admin.groups.accountsAvailable', {
                      count: row.active_account_count || 0
                    })
                  }}
                </span>
                <span class="rounded bg-amber-50 px-2 py-0.5 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                  {{
                    t('admin.groups.accountsRateLimited', {
                      count: row.rate_limited_account_count || 0
                    })
                  }}
                </span>
              </div>
              ...
              <div class="space-y-1">
                <div class="flex items-center justify-between gap-2 text-gray-600 dark:text-gray-400">
                  ...
                  <span class="font-medium text-gray-800 dark:text-gray-200">
                    {{ capacityMap[row.id]?.concurrency_used || 0 }}/{{ capacityMap[row.id]?.concurrency_max || 0 }}
                  </span>
                </div>
                <div class="h-1.5 rounded-full bg-gray-100 dark:bg-dark-600">
                  <div
                    class="h-1.5 rounded-full bg-sky-500 transition-all"
                    :style="{ width: `${capacityPercent(capacityMap[row.id]?.concurrency_used, capacityMap[row.id]?.concurrency_max)}%` }"
                  ></div>
                </div>
              </div>
              <div class="grid grid-cols-2 gap-1 text-[11px] text-gray-500 dark:text-gray-400">
                <span>
                  Session {{ capacityMap[row.id]?.sessions_used || 0 }}/{{ capacityMap[row.id]?.sessions_max || 0 }}
                </span>
                <span class="text-right">
                  RPM {{ capacityMap[row.id]?.rpm_used || 0 }}/{{ capacityMap[row.id]?.rpm_max || 0 }}
                </span>
              </div>
            </div>
          </template>
          ...
          <template #cell-usage="{ row }">
            <div class="space-y-1 text-xs">
              <div class="flex items-center justify-between gap-3">
                <span class="text-gray-500 dark:text-gray-400">{{ t('admin.groups.usageToday') }}</span>
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ formatCost(usageMap[row.id]?.today_cost) }}
                </span>
              </div>
              <div class="flex items-center justify-between gap-3">
                <span class="text-gray-500 dark:text-gray-400">{{ t('admin.groups.usageTotal') }}</span>
                <span class="font-medium text-gray-700 dark:text-gray-300">
                  {{ formatCost(usageMap[row.id]?.total_cost) }}
                </span>
              </div>
            </div>
          </template>
      ...
      <form id="create-group-form" @submit.prevent="handleCreateGroup" class="space-y-5">
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-400">
          <div class="flex items-center justify-between">
            <div>
              <label class="font-medium text-gray-900 dark:text-white">允许 Chatbot</label>
              <p class="text-sm text-gray-500 dark:text-gray-400">
                开启后，该分组下的 APIKey 才会显示「聊天」入口，并可在 Chatbot 中选择。
              </p>
            </div>
            <button
              type="button"
              @click="createForm.chatbot_enabled = !createForm.chatbot_enabled"
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                createForm.chatbot_enabled ? 'bg-primary-500' : 'bg-gray-300 dark:bg-dark-600'
              ]"
            >
              <span
                :class="[
                  'inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                  createForm.chatbot_enabled ? 'translate-x-6' : 'translate-x-1'
                ]"
              />
            </button>
          </div>
        ...
        <div v-if="createForm.platform === 'openai'" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4">
          ...
          <div v-if="createForm.allow_messages_dispatch" class="mt-3">
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-3">
              <div>
                <label class="input-label">Opus Target</label>
                <input v-model="createForm.opus_mapped_model" type="text" class="input" placeholder="gpt-5.4" />
              </div>
              <div>
                <label class="input-label">Sonnet Target</label>
                <input v-model="createForm.sonnet_mapped_model" type="text" class="input" placeholder="gpt-5.3-codex" />
              </div>
              <div>
            ...
            <div class="mt-4 rounded-lg border border-gray-200 p-3 dark:border-dark-600">
              <div class="mb-2 flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Exact Claude Model Overrides</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">Specific Claude model mappings override the family defaults above.</p>
                </div>
                <button type="button" class="btn btn-secondary btn-sm" @click="addCreateMessagesDispatchMapping">
                  <Icon name="plus" size="sm" class="mr-1" />
                  Add
                </button>
              </div>
              <div v-if="createForm.exact_model_mappings.length === 0" class="text-xs text-gray-500 dark:text-gray-400">
              ...
              <div v-else class="space-y-2">
                ...
                >
                  <input v-model="row.claude_model" type="text" class="input" placeholder="claude-sonnet-4-5-20250929" />
                  <input v-model="row.target_model" type="text" class="input" placeholder="gpt-5.3-codex" />
                  <button type="button" class="btn btn-secondary" @click="removeCreateMessagesDispatchMapping(row)">
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
              </div>
      ...
      >
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-400">
          <div class="flex items-center justify-between">
            ...
            <button
              type="button"
              @click="editForm.chatbot_enabled = !editForm.chatbot_enabled"
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                editForm.chatbot_enabled ? 'bg-primary-500' : 'bg-gray-300 dark:bg-dark-600'
              ]"
            >
              <span
                :class="[
                  'inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                  editForm.chatbot_enabled ? 'translate-x-6' : 'translate-x-1'
                ]"
              />
            </button>
          </div>
        ...
        <div v-if="editForm.platform === 'openai'" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4">
          ...
          <div v-if="editForm.allow_messages_dispatch" class="mt-3">
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-3">
              <div>
                <label class="input-label">Opus Target</label>
                <input v-model="editForm.opus_mapped_model" type="text" class="input" placeholder="gpt-5.4" />
              </div>
              <div>
                <label class="input-label">Sonnet Target</label>
                <input v-model="editForm.sonnet_mapped_model" type="text" class="input" placeholder="gpt-5.3-codex" />
              </div>
              <div>
                <label class="input-label">Haiku Target</label>
            ...
            <div class="mt-4 rounded-lg border border-gray-200 p-3 dark:border-dark-600">
              <div class="mb-2 flex items-center justify-between gap-3">
                ...
                <button type="button" class="btn btn-secondary btn-sm" @click="addEditMessagesDispatchMapping">
                  <Icon name="plus" size="sm" class="mr-1" />
                  Add
                </button>
              </div>
              <div v-if="editForm.exact_model_mappings.length === 0" class="text-xs text-gray-500 dark:text-gray-400">
                No exact mappings configured.
              </div>
              <div v-else class="space-y-2">
                <div
                ...
                >
                  <input v-model="row.claude_model" type="text" class="input" placeholder="claude-sonnet-4-5-20250929" />
                  <input v-model="row.target_model" type="text" class="input" placeholder="gpt-5.3-codex" />
                  <button type="button" class="btn btn-secondary" @click="removeEditMessagesDispatchMapping(row)">
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
              </div>
          ...
          >
            ...
            <div class="flex-1">
              ...
              <div class="text-xs text-gray-500 dark:text-gray-400">
                ...
                >
                  {{ group.platform === 'gpt-image' ? 'GPT-Image' : t('admin.groups.platforms.' + group.platform) }}
                </span>
              </div>
            </div>
...
const usageMap = ref<Record<number, { today_cost: number; total_cost: number }>>({})
const capacityMap = ref<Record<number, {
...
const capacityPercent = (used?: number | null, max?: number | null) => {
  const denominator = Number(max || 0)
  if (denominator <= 0) return 0
  const pct = (Number(used || 0) / denominator) * 100
...
const loadUsageSummary = async () => {
  ...
  try {
    const items = await adminAPI.groups.getUsageSummary(browserTimezone || undefined)
    usageMap.value = Object.fromEntries(items.map((item) => [item.group_id, item]))
  } catch (error) {
...
const loadCapacitySummary = async () => {
  ...
  try {
    const items = await adminAPI.groups.getCapacitySummary()
    capacityMap.value = Object.fromEntries(items.map((item) => [item.group_id, item]))
  } catch (error) {
...
const loadGroups = async () => {
  ...
  try {
    ...
    await Promise.all([loadUsageSummary(), loadCapacitySummary()])
  } catch (error: any) {
...
const closeCreateModal = () => {
  ...
  createForm.chatbot_enabled = false
...
const handleCreateGroup = async () => {
  ...
  try {
    ...
    const requestData = {
      ...
      gpt_image_call_price: normalizeOptionalLimit(createForm.gpt_image_call_price as number | string | null),
...
const handleEdit = async (group: AdminGroup) => {
  ...
  editForm.chatbot_enabled = group.chatbot_enabled || false
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      gpt_image_call_price: normalizeOptionalLimit(editForm.gpt_image_call_price as number | string | null),
