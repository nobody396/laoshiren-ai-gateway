...
func (h *DashboardHandler) GetUserBreakdown(c *gin.Context) {
	...
	dim.Model = strings.TrimSpace(c.Query("model"))
	...
	dim.Endpoint = strings.TrimSpace(c.Query("endpoint"))
	dim.EndpointType = strings.TrimSpace(c.DefaultQuery("endpoint_type", "inbound"))
	...
	stats, err := h.dashboardService.GetUserBreakdownStats(c.Request.Context(), startTime, endTime, dim, limit)
	if err != nil {
