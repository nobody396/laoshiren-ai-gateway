...
func (s *UsageLogRepoSuite) TestDashboardStats_TodayTotalsAndPerformance() {
	...
	s.Require().InEpsilon(baseStats.TotalUserBalance+20.75, stats.TotalUserBalance, 0.0001, "TotalUserBalance mismatch")
	...
	s.Require().Equal(baseStats.YesterdayActualCost+0.7, stats.YesterdayActualCost, "YesterdayActualCost mismatch")
...
func (s *UsageLogRepoSuite) TestDashboardStatsWithRange_Fallback() {
	...
	s.Require().Equal(0.9, stats.YesterdayActualCost)
