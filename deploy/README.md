...
curl -sSL https://raw.githubusercontent.com/bozhouDev/DragonCode-sub2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
# Admin password is generated into .env by docker-deploy.sh
...
1. On first run, the system automatically:
   ...
   - Creates admin account using `ADMIN_EMAIL` and `ADMIN_PASSWORD`
   - Writes config.yaml

2. No manual Setup Wizard needed - just configure `.env` and start

3. Set `ADMIN_PASSWORD` before first startup. The helper script writes it to `.env` automatically:
   ```bash
   grep '^ADMIN_PASSWORD=' .env
   ```
...
1. Download the latest release from [GitHub Releases](https://github.com/bozhouDev/DragonCode-sub2api/releases)
