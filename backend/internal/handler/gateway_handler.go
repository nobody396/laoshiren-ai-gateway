...
import (
	...
	pkgerrors "github.com/bozhouDev/DragonCode-sub2api/internal/pkg/errors"
...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		for {
			...
			} else if account.Platform == service.PlatformAnthropic && account.Type == service.AccountTypeAPIKey &&
				h.gatewayService.ShouldEmulateWebSearch(c.Request.Context(), account, currentAPIKey.GroupID, parsedReq.Body) {
				result, err = h.gatewayService.HandleWebSearchEmulation(requestCtx, c, account, parsedReq)
			} else {
			...
			// Successful streams and interrupted streams with provider-observed
			// usage share the same durable accounting path.
...
func (h *GatewayHandler) MultiGroupModels(ctx context.Context, group *service.Group) ([]string, error) {
	return h.gatewayService.MultiGroupModels(ctx, group)
}
