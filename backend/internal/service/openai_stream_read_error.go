...
func newOpenAIUpstreamStreamReadError(err error) error {
	...
	return &openAIUpstreamStreamReadError{cause: err, clientCode: code, clientMessage: message}
}

func shouldClassifyOpenAIUpstreamStreamReadError(err error, contexts ...context.Context) bool {
	if err == nil || errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) ||
		errors.Is(err, ErrUpstreamResponseBodyTooLarge) || errors.Is(err, bufio.ErrTooLong) {
		return false
	}
