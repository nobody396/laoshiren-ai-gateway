...
type SystemSettings struct {
	...
	APIBaseURL                  string
	ContactInfo                 string
	TechSupportQRCode           string // 技术客服二维码图片 URL
	AfterSalesQRCode            string // 售后客服二维码图片 URL
	DocURL                      string
	ChatbotURL                  string
	HomeContent                 string
	...
	StripeSecretKeyConfigured     bool
	StripeWebhookSecret           string
	StripeWebhookSecretConfigured bool
	...
	BalanceAlertDefaultThreshold float64
	...
	XunhuAlipayKey           string
	XunhuAlipayKeyConfigured bool
...
type OpenAIFastPolicyRule struct {
	ServiceTier          string   `json:"service_tier"`                     // "priority" | "flex" | "all"
	Action               string   `json:"action"`                           // "pass" | "filter" | "block"
	Scope                string   `json:"scope"`                            // "all" | "oauth" | "apikey" | "bedrock"
	ErrorMessage         string   `json:"error_message,omitempty"`          // 自定义错误消息 (action=block 时生效)
	ModelWhitelist       []string `json:"model_whitelist,omitempty"`        // 模型匹配模式列表（为空=对所有模型生效）
...
// OpenAIFastPolicySettings OpenAI fast/flex 策略配置
...
// 默认对所有模型的 priority（fast）请求执行 filter，让上游按默认优先级处理。
