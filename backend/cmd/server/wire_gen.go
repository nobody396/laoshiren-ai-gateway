...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, teamRepository, configConfig)
	...
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, accountQuotaAlertService, balanceAlertService, commissionService, gptImageTaskRepository, gptImageS3Storage, settingService, accountingService, openAIRouteController, openAIRouteAuditService, openAIRouteObservationCollector, reliabilityEvidenceService)
	...
	publicStatsCache := repository.NewPublicStatsCache(redisClient, configConfig)
	publicStatsService := service.ProvidePublicStatsService(dashboardAggregationRepository, redeemCodeRepository, publicStatsCache)
	...
	easyPayClient := service.ProvideEasyPayClient(settingService)
	registry := service.ProvidePaymentRegistry(easyPayClient)
	topupService := service.NewTopupService(topupOrderRepository, settingService, userRepository, accountChangeRecordRepository, client, billingCacheService, apiKeyAuthCacheInvalidator, commissionService, balanceAlertService, affiliateConsumptionRepository, affiliateRewardService, registry)
	topupHandler := handler.NewTopupHandler(topupService)
	topupEasyPayNotifier := handler.ProvideTopupEasyPayNotifier(topupService)
	...
	nativeCheckoutEasyPayNotifier := handler.ProvideNativeCheckoutEasyPayNotifier(nativeCheckoutService)
	paymentGatewayHandler := handler.NewPaymentGatewayHandler(registry, topupEasyPayNotifier, nativeCheckoutEasyPayNotifier)
	...
	statusHandler := handler.ProvideStatusHandler(statusControlService, incidentControlService)
	customerTierService := service.NewCustomerTierService(db, settingRepository)
	customerTierHandler := handler.NewCustomerTierHandler(customerTierService)
	compensationControlService := service.NewCompensationControlService(db, settingRepository, customerTierService)
	compensationExecutionService := service.NewCompensationExecutionService(db, settingRepository, compensationControlService, billingCacheService)
	compensationHandler := handler.NewCompensationHandler(compensationControlService, compensationExecutionService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, agentHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, changelogHandler, invoiceHandler, feedbackHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, modelPricingHandler, publicStatsHandler, totpHandler, paymentHandler, topupHandler, paymentGatewayHandler, nativeCheckoutHandler, balanceAlertHandler, resourceHandler, statusHandler, customerTierHandler, compensationHandler, handlerTeamHandler, idempotencyCoordinator, idempotencyCleanupService, pendingAuthSessionCleanupService)
	...
	lifecycle := service.ProvideRootLifecycle(configConfig, accountRepository, pricingService, apiKeyService, billingCacheService, emailQueueService, subscriptionService, accountingWorker, usageRecordWorkerPool, openAIRouteAuditService, openAIRouteObservationCollector, reliabilityEvidenceService, statusControlService, incidentControlService, customerTierService, compensationControlService, timingWheelService, dashboardAggregationService, deferredService, schedulerSnapshotService, concurrencyService, userMessageQueueService, tokenRefreshService, accountExpiryService, subscriptionExpiryService, usageCleanupService, agentLevelEvaluatorService, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, idempotencyCleanupService, scheduledTestRunnerService, downloadResourceService, backupService, pendingAuthSessionCleanupService, affiliateRewardService, affiliateAgentActivationScheduler, nativeCheckoutService)
