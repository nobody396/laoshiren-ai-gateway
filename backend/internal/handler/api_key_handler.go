...
func validateAPIKeyCreateRequest(req CreateAPIKeyRequest) error {
	for _, value := range []*float64{req.Quota, req.RateLimit5h, req.RateLimit1d, req.RateLimit7d} {
		if value != nil && !validAPIKeyLimit(*value) {
...
func (h *APIKeyHandler) GetAvailableGroups(c *gin.Context) {
	...
	groups, err := h.apiKeyService.GetAvailableGroupsForScope(c.Request.Context(), subject.UserID, c.Query("scope"))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// GetAvailableGroupCacheStats 获取用户可用分组的缓存命中率统计
// GET /api/v1/groups/cache-stats?window_days=7
func (h *APIKeyHandler) GetAvailableGroupCacheStats(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	if raw := strings.TrimSpace(c.Query("window_days")); raw != "" {
		...
		if err != nil || parsed <= 0 {
			response.BadRequest(c, "Invalid window_days")
			return
		}
		windowDays = parsed
	}
	...
	stats, err := h.apiKeyService.GetAvailableGroupCacheStats(c.Request.Context(), subject.UserID, windowDays)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	out := make(map[int64]gin.H, len(stats))
	for groupID, stat := range stats {
		out[groupID] = gin.H{
			"hit_rate":     stat.HitRate,
			"hit_rate_pct": stat.HitRatePct,
			"has_data":     stat.HasData,
		}
	}
