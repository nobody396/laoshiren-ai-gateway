...
export const useAuthStore = defineStore('auth', () => {
  ...
  function scheduleTokenRefreshAt(expiresAtMs: number): void {
    ...
    if (refreshInMs <= 0) {
      // Token is already expired — do NOT proactively refresh here.
      // refreshUser() is already in flight (called from checkAuth), and the
      // client.ts interceptor will handle the resulting 401 via its own refresh
      // path. Calling performTokenRefresh() simultaneously causes two concurrent
      // POST /auth/refresh requests with the same single-use rotating refresh
      // token: the second request will be rejected (REFRESH_TOKEN_INVALID) and
      // the interceptor will then clear auth and redirect to /login.
      return
    }
