...
const showShellTabs = computed(() => activeClientTab.value !== 'opencode' && props.platform !== 'gpt-image')
...
const currentFiles = computed((): FileConfig[] => {
  ...
  const gptImageBase = ensureV1(`${baseRoot}/gpt-image`)
...
function generateGPTImageFiles(baseUrl: string, apiKey: string): FileConfig[] {
  const endpoint = `${baseUrl}/images/generations`
  const taskEndpoint = `${baseUrl}/tasks/task_xxx`
  const curlContent = `curl '${endpoint}' \\
  ...
  const pollContent = `curl '${taskEndpoint}' \\
  -H 'Authorization: Bearer ${apiKey}'`

  const pythonContent = `import time
import requests
...
while True:
    ...
    if data["status"] == "completed":
        print(data["result"]["images"][0]["url"][0])
        break
  ...
  return [
    {
      path: 'cURL - submit task',
      content: curlContent,
      hint: t('keys.useKeyModal.gptImage.endpointHint', { baseUrl })
    },
    {
      path: 'cURL - query task',
      content: pollContent,
      hint: t('keys.useKeyModal.gptImage.pollHint')
    },
    {
