...
// mode is optional; "compact" probes OpenAI /responses/compact support.
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID int64, modelID string, prompt string, modeValues ...string) error {
	...
	if account.IsOpenAI() {
		return s.testOpenAIAccountConnectionWithMode(c, account, modelID, prompt, normalizeAccountTestMode(mode))
	}
...
// testOpenAIAccountConnectionWithMode tests an OpenAI account's connection.
func (s *AccountTestService) testOpenAIAccountConnectionWithMode(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	ctx := c.Request.Context()
	_ = prompt
	...
	if account.IsOAuth() {
		...
		if !openai_compat.ShouldUseResponsesAPI(account.Extra) {
			return s.sendErrorAndEnd(c, "This OpenAI-compatible upstream does not support /v1/responses; runtime Chat Completions traffic will be forwarded directly.")
		}
