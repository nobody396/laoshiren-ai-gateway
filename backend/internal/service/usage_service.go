...
func ProvideUsageService(usageRepo UsageLogRepository, userRepo UserRepository, entClient *dbent.Client, authCacheInvalidator APIKeyAuthCacheInvalidator, gptImageTaskRepo GPTImageTaskRepository) *UsageService {
	svc := NewUsageService(usageRepo, userRepo, entClient, authCacheInvalidator)
	svc.gptImageTaskRepo = gptImageTaskRepo
	return svc
}
...
func (s *UsageService) GetByID(ctx context.Context, id int64) (*UsageLog, error) {
	...
	logs := []UsageLog{*log}
	if err := s.enrichGPTImageResults(ctx, logs); err != nil {
		return nil, fmt.Errorf("enrich gpt-image results: %w", err)
	}
	*log = logs[0]
	return log, nil
}
...
func (s *UsageService) ListByUser(ctx context.Context, userID int64, params pagination.PaginationParams) ([]UsageLog, *pagination.PaginationResult, error) {
	...
	if err := s.enrichGPTImageResults(ctx, logs); err != nil {
		return nil, nil, fmt.Errorf("enrich gpt-image results: %w", err)
	}
...
func (s *UsageService) enrichGPTImageResults(ctx context.Context, logs []UsageLog) error {
	if s == nil || s.gptImageTaskRepo == nil || len(logs) == 0 {
		return nil
	}

	taskIDs := make([]string, 0)
	logIndexByTaskID := make(map[string][]int)
	for i := range logs {
		taskID := usageLogGPTImageTaskID(logs[i])
		if taskID == "" {
			continue
		}
		if _, exists := logIndexByTaskID[taskID]; !exists {
			taskIDs = append(taskIDs, taskID)
		}
		logIndexByTaskID[taskID] = append(logIndexByTaskID[taskID], i)
	}
	if len(taskIDs) == 0 {
		return nil
	}

	tasks, err := s.gptImageTaskRepo.ListStoredByTaskIDs(ctx, taskIDs)
	if err != nil {
		return err
	}
	for taskID, task := range tasks {
		urls := buildGPTImageUsageResultURLs(task)
		if len(urls) == 0 {
			continue
		}
		for _, idx := range logIndexByTaskID[taskID] {
			if task.UserID != logs[idx].UserID || task.APIKeyID != logs[idx].APIKeyID {
				continue
			}
			logs[idx].ImageResultURLs = urls
		}
	}
	return nil
}

func usageLogGPTImageTaskID(log UsageLog) string {
	if log.ImageCount <= 0 {
		return ""
	}
	requestID := strings.TrimSpace(log.RequestID)
	taskID := strings.TrimPrefix(requestID, "gpt-image-task:")
	if taskID == requestID || taskID == "" {
		return ""
	}
	return taskID
}

func buildGPTImageUsageResultURLs(task *GPTImageTask) []string {
	if task == nil || strings.TrimSpace(task.TaskID) == "" || strings.TrimSpace(task.MediaToken) == "" || len(task.S3ObjectKeys) == 0 {
		return nil
	}
	urls := make([]string, 0, len(task.S3ObjectKeys))
	escapedTaskID := url.PathEscape(task.TaskID)
	escapedToken := url.QueryEscape(task.MediaToken)
	for i := range task.S3ObjectKeys {
		urls = append(urls, fmt.Sprintf("/gpt-image/media/%s/%d?token=%s", escapedTaskID, i, escapedToken))
	}
	return urls
}
