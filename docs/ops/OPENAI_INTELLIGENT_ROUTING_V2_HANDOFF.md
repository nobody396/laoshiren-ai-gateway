...
9. 共享观测热路径增加 5 秒有界 L1 与 singleflight：相同候选集合并发只回源一次，
   20ms 超时、错误不缓存、深拷贝返回，Redis 仍是权威源，健康熔断仍每次评估读取；缓存
...
12. 24 小时只作为健康检查点，自动证据主评估要求查询窗口达到 72 小时；考虑查询
    右边界不包含且真实流量不与边界同步，实际首末决策跨度至少 71 小时（两端合计
    空档最多 1 小时）；延期复评扩大查询窗口时仍只允许合计 1 小时空档，防止宽窗口
    稀释短时突发。响应给出确定性的 24h/72h/失败后 24h 复评计划，但不创建定时器且固定
...
16. 审计与被动采集完整率计数器记录本实例启动时间；若实例在 Shadow T0 之后启动，
    `health_counter_coverage` 必须阻止该切片晋级，避免重启把历史丢失归零成 100%。
    多实例部署在持久化集群级计数器完成前仍需逐实例人工核验；启动后的审计/观测
    存储探针必须保持零失败，后续恢复不能抹掉此前的证据空档。
