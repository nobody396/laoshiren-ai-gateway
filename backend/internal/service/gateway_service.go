...
type AccountSelectionResult struct {
	...
	OpenAIRouteShadow *OpenAIRouteShadowDecision
}
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, _ ...int64) (*AccountSelectionResult, error) {
	// 调试日志：记录调度入口参数
...
// UA 必须匹配官方 CLI，metadata.user_id 也必须符合 Claude Code 格式。
...
// rewriteSystemForNonClaudeCode migrates a third-party client's system prompt
// into messages and replaces system with Claude Code billing attribution plus
// the canonical Claude Code prompt.
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.isThinkingBlockSignatureError(respBody) && s.settingService.IsSignatureRectifierEnabled(ctx) {
					...
					releaseRetryCtx()
					if buildErr == nil {
						retryResp, retryErr := s.httpUpstream.DoWithTLS(retryReq, proxyURL, account.ID, account.Concurrency, s.resolveTLSProfile(account))
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isThinkingBlockSignatureError(retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := s.httpUpstream.DoWithTLS(retryReq2, proxyURL, account.ID, account.Concurrency, s.resolveTLSProfile(account))
										if retryErr2 == nil {
											resp = retryResp2
											break
										}
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							budgetRetryResp, retryErr := s.httpUpstream.DoWithTLS(budgetRetryReq, proxyURL, account.ID, account.Concurrency, s.resolveTLSProfile(account))
							if retryErr == nil {
	...
	if c != nil && strings.TrimSpace(mappedModel) != "" {
		c.Set("ops_upstream_model", strings.TrimSpace(mappedModel))
	}
...
func (s *GatewayService) forwardAnthropicAPIKeyPassthrough(
	...
	if c != nil && strings.TrimSpace(reqModel) != "" {
		c.Set("ops_upstream_model", strings.TrimSpace(reqModel))
	}
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, error) {
	...
	// 白名单透传 headers。
	// OAuth mimicry 路径跳过客户端 header 透传，避免 user-agent /
	// anthropic-beta / x-stainless-* 等客户端痕迹和网关注入的 Claude Code
	// 指纹互相冲突。
...
// sanitizeStreamError returns a client-safe stream error description without
// internal source/destination addresses from net.OpError.
...
// partialStreamUsageResult preserves upstream-observed usage when a stream
// ends after the provider has already metered tokens. Failover errors remain
// result-less so a later successful attempt cannot be billed twice.
...
func (s *GatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	for {
		...
		case <-intervalCh:
			...
			sendErrorEvent("stream_timeout", "upstream stream timed out")
			return &streamingResult{usage: usage, firstTokenMs: firstTokenMs}, fmt.Errorf("stream data interval timeout")
...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	cost := p.Cost
	subscriptionCost := subscriptionUsageCost(p)
	...
	// 代理商消耗分佣统一由调用方（写完 usage_log 后）触发 triggerConsumptionCommission，
	// 便于 source_id 关联到 usage_logs.id，实现运行态与回填工具共用的幂等语义。
...
func subscriptionUsageCost(p *postUsageBillingParams) float64 {
	if p == nil || p.Cost == nil || !p.IsSubscriptionBill {
		return 0
	}
	if p.APIKey != nil && p.APIKey.Group != nil && p.APIKey.Group.IsCreditSubscription() {
...
func resolveUsageBillingRequestID(ctx context.Context, upstreamRequestID string) string {
	if requestID := strings.TrimSpace(upstreamRequestID); strings.HasPrefix(requestID, "gpt-image-task:") {
		return requestID
	}
	if ctx != nil {
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	subscriptionCost := subscriptionUsageCost(p)
	if p.IsSubscriptionBill && p.Subscription != nil && subscriptionCost > 0 {
		cmd.SubscriptionID = &p.Subscription.ID
		cmd.SubscriptionCost = subscriptionCost
...
// triggerConsumptionCommission 在 usage_log 落库、账单成功应用后异步写入代理商消耗分佣。
//
// 前置条件：
//   - applied 为 true（未被事务化计费去重）
//   - 非订阅/套餐扣费（订阅按现有产品策略不分佣）
//   - 有非零 ActualCost
//   - usageLog.ID 已赋值（作为 source_id 支撑幂等）
//
// 写入侧依赖数据库上 (type, source_id) 的部分唯一索引保证幂等，
...
func triggerConsumptionCommission(applied bool, usageLog *UsageLog, p *postUsageBillingParams, deps *billingDeps) {
	...
	if p.Cost == nil || p.Cost.ActualCost <= 0 {
		return
	}
	if p.User == nil {
		return
	...
	if usageLog == nil || usageLog.ID == 0 {
		// 缺少 usage_log.id 等同于无幂等键，跳过以避免重复写入风险；
		// 后续可由 commission_backfill 按 usage_logs 兜底。
		slog.Warn("skip consumption commission: missing usage_log id",
			"user_id", p.User.ID, "actual_cost", p.Cost.ActualCost)
		return
	}

	userID := p.User.ID
	actualCost := p.Cost.ActualCost
	sourceID := usageLog.ID
	go func() {
		commCtx, commCancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer commCancel()
		if err := deps.commissionService.ProcessConsumptionCommission(commCtx, userID, actualCost, sourceID); err != nil {
			slog.Error("process consumption commission failed",
				"user_id", userID, "source_id", sourceID, "error", err)
		}
	}()
}
...
func notifyBalanceLow(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p == nil || deps == nil || p.Cost == nil || p.IsSubscriptionBill || p.Cost.ActualCost <= 0 || p.User == nil || deps.balanceAlertService == nil {
		return
	}
	balanceAfterDeduct := p.User.Balance - p.Cost.ActualCost
	if result != nil && result.NewBalance != nil {
		balanceAfterDeduct = *result.NewBalance
	}
...
func notifyAccountQuota(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p == nil || deps == nil || p.Cost == nil || p.Cost.TotalCost <= 0 || p.Account == nil || !p.Account.IsAPIKeyOrBedrock() || deps.accountQuotaAlertService == nil {
		return
	}
	...
	deps.accountQuotaAlertService.CheckAccountQuotaAfterIncrement(context.Background(), p.Account, accountCost, quotaState)
}
...
func (s *GatewayService) buildCountTokensRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, mimicClaudeCode bool) (*http.Request, error) {
	...
	// 白名单透传 headers。OAuth mimicry 路径跳过客户端 header 透传。
	if tokenType != "oauth" || !mimicClaudeCode {
		for key, values := range clientHeaders {
			lowerKey := strings.ToLower(key)
			if allowedHeaders[lowerKey] {
